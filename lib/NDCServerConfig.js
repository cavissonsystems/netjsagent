...
NDCServerConfig.resetserverlist = function(){
    NDCServerConfig.serverList = [];
    NDCServerConfig.currentActiveServer = undefined;
}
...
NDCServerConfig.addServers = function(ndcAddress){
    try {
        ...
        for (var i in ndcAddress) {
            ...
            temp = makeServerObject(ndcAddress[i].host, ndcAddress[i].port, ndcAddress[i].type, i,ndcAddress[i].protocols,ndcAddress[i].isSaaSserver)
            if (temp) {
                if(temp.type == 1)
...
NDCServerConfig.switchToSaaS = function() {
    try{
        var agentSetting = require('./agent-setting');
        if(agentSetting.requestType == 2){
        ...
        util.logger.info(agentSetting.currentTestRun + " | After all retry to connect Master host and port  adding Saas again to  server list");
        util.logger.info(agentSetting.currentTestRun + " | Server list data --->> ", JSON.stringify(this.serverList));
    }
    catch (e) {
        util.logger.warn('0| Error in switchToSaaS',e)
    }
}
...
NDCServerConfig.switchNDCMode = function(){
    try{
        var agentSetting = require('./agent-setting');
        var NDCManager = require('./NDConnectionManager');
...
function makeServerObject(ndcHost , ndcPort , type , index,protocols,isSaaSserver){
    ...
    if(ndcHost == undefined || ndcHost == null || ndcHost == 0 || ndcPort == undefined || ndcPort == null || ndcPort == 0 )
        ...
        return new Server(ndcHost , ndcPort , type , index,protocols,isSaaSserver)
}


function Server(ndcHost,ndcPort,type,index,protocols,isSaaSserver){
    var agentSetting = require('./agent-setting')
    ...
    this.index = index;
    this.protocolList = []
    this.isSaaSserver = isSaaSserver;
    var list = protocols ? protocols.split(',') :agentSetting.isSaaSMode ? ['WSS'] : ['TCP']                //Creating array of all protocols
