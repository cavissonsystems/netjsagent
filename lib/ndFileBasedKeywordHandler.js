...
NDFileBasedKeywordHandler.readFileContent = function (clientMsg) {
    try {
        ...
        var mthObj = {"keywordName":keyword,"size":size,"lmd":lmd,type:'file'};
        return mthObj;
    }
    catch (err) {
        util.logger.warn(err);
...
NDFileBasedKeywordHandler.readInstrProfileContent = function (allFields) {
    try {
        var KeywithValue = [],size,lmd,keyword;
        //clientMsg = "action=instrumentation_profile;instrProfile=nodeInstr.json;size=283;lmd=1497703370";
        for (var i=0 ; i< allFields.length ;i++) {
            if (allFields[i].toString().startsWith("InstrProfile=") || allFields[i].toString().indexOf("InstrProfile=/") != -1 || allFields[i].toString().startsWith("instrProfile=") || allFields[i].toString().indexOf("instrProfile=") != -1) {
                keyword=allFields[i].toString().split('=')[1]
                if (allFields[i+1].toString().indexOf("size") != -1) {
                    size = allFields[i+1].toString().split('=')[1]
                }

                if (allFields[i+2].toString().indexOf("lmd") != -1) {
                    lmd = allFields[i+2].toString().split('=')[1]
                }
                break;
            }
        }
        return {"keywordName":keyword,"size":size,"lmd":lmd,type:"instrumentationProfile"};
    }
    catch (err) {
        util.logger.warn(err);
    }
