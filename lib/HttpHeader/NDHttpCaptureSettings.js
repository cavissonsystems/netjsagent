...
NDHttpCaptureSettings.dumpHttpReqResHeader= function(req,res,flowpathobj) {
    ...
    if (reqCapture && 3 == reqCapture.captureLevel)
            this.dumpHttpReqHeaders(req, reqCapture,flowpathObj); // generate 6 record and dump them.
    if (respCapture && 2 == respCapture.captureLevel)
            this.dumpHttpRespHeaders(res, respCapture,flowpathObj); // generate 6 record and dump them.

    NDHttpConditionStats.updateMonitorCount(req,res,reqCapture,respCapture)
}

NDHttpCaptureSettings.dumpHttpRespHeaders= function(res ,respCapture,flowpathObj) {
    ...
    if(0 == respCapture.hdrMode) // 0 means all headers to be captured.
        this.getAllHeadersAndValuesForResponse(res, respCapture,respHeaderStr,flowpathObj,respHeaderCallback);
    else
        this.getSpecifiedHeadersForResponse(res, respCapture,respHeaderStr,flowpathObj,respHeaderCallback); // capture only specified headers and their values for response, , null is for request object, false is to determine that it is not request.
}
 ...
 */
NDHttpCaptureSettings.getSpecifiedHeadersForResponse = function (res, respCapture,headerRecordStr,flowpathObj,respHeaderCallback) {
    try {
 ...
 * @param httpServletResponseObj
 */
NDHttpCaptureSettings.getAllHeadersAndValuesForResponse = function (res, respCapture,headerRecordStr,flowpathObj,respHeaderCallback) {
    try{
        var headers,headerName,headerValue
        headers = res['_headers']
...
NDHttpCaptureSettings.dumpHttpReqHeaders= function(req ,reqCapture,flowpathObj) {
    ...
    if(0 == reqCapture.hdrMode) // 0 means all headers to be captured.
        NDHttpCaptureSettings.getAllHeadersAndValuesForRequest(req, reqCapture,headerRecordStr,flowpathObj);
    else
        NDHttpCaptureSettings.getSpecifiedHeadersForRequest(req, reqCapture,headerRecordStr,flowpathObj); // capture only specified headers and their values, null is for response object, true is to determine request.
}
...
NDHttpCaptureSettings.getAllHeadersAndValuesForRequest = function(req,reqCapture,headerRecordStr,flowpath){
    var headername,
        headerValue,
        headers,
        headerValueList,
    ...
    for(var i in headers){
        ...
        if(!getHdrSet[headername])
            headerRecordStr = this.appendHeaderNameIdAndValue(headerNameId, headerValue, 12,headerRecordStr,flowpath);
        else {
            // getting header value's id
            var headerValueId = this.getIDAndDumpHttpMetaRecord(headerValue);
            headerRecordStr = this.appendHeaderNameIdAndValueId(headerNameId, headerValueId, 12,headerRecordStr,flowpath);
        }
        //}
        //if(ndHttpCptureTraceLevel > 4)
...
NDHttpCaptureSettings.getSpecifiedHeadersForRequest =function(req, reqCapture,headerRecordStr,flowpathObj){
    //if(ndHttpCptureTraceLevel > 4)
    ...
    try {
        for(var i in reqCapture.hdrNameList){
            ...
            //for(var value in headerValueList) {
            //    headerValue = headerValueList[value]
            var headerNameId = this.getIDAndDumpHttpMetaRecord(headername);
            if (0 == reqCapture.hdrValueLengthMode)
                headerValue = this.encodeURL(headerValue, 0); // doing the url encoding for full value.
            else
                headerValue = this.encodeURL(headerValue, reqCapture.hdrValueMaxLength); // truncation the value with specified length and then do the url encoding.

            //if(NDHttpConditionStats.getHttpMonitorEnabled())
            //    saveHdrValAtConditionIndex(NDHttpConditionStats.httpStatsCondReqHdrsMap, headerName, headerValueForCondition);
            if (!getHdrSet[headername])
                headerRecordStr = this.appendHeaderNameIdAndValue(headerNameId, headerValue, 12, headerRecordStr, flowpathObj);
            else {
                // getting header value's id
                var headerValueId = this.getIDAndDumpHttpMetaRecord(headerValue);
                headerRecordStr = this.appendHeaderNameIdAndValueId(headerNameId, headerValueId, 12, headerRecordStr, flowpathObj);
            }
            //}

        }

        this.dumpMappingOfHdrNameIdWithValue(true,headerRecordStr,flowpathObj);
    }
    catch(e) {
        console.log(e)
