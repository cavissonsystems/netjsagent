...
function Callback(headerName,alisaName){
    try {
        ...
        //NDListener.logBCIError(Server.TestRunIDValue, "NDHttpConditionStats", "createTargetedReqAndRespHeadersList", "Error occured while creating condition headers for request and response types", e);
    }
}
...
NDHttpConditionStats.manageAllSpecifiedConditions=function(headerList){
    try{
        if(!headerList){
            //if(ndHttpCptureTraceLevel > 0)
            //    NDListener.logBCITrace(Server.TestRunIDValue, "", "", "No condiitons found for monitoring.");
            NDHttpConditionStats.isHttpConditionEnabled = false;
            return;
        }
        this.resetValues();
        ...
        for( var i in headerList) {
            ...
            var conditionStr=''
            if(allFields[1].toString().trim().indexOf(COND_COOKIE_TYPE_REQ) != -1)
                isCookieMonEnabled = true;
            if (allFields.length > 1)
                conditionStr = Callback(allFields[1].toString().trim(),allFields[0].toString().trim())
            else{
                //if(ndHttpCptureTraceLevel > 1)
                //    NDListener.logBCITrace(Server.TestRunIDValue, "", "", "ERROR:- Invalid Condition Expression found In current Line = " + currLine);
                continue;
            }
        }
        ...
        NDHttpConditionStats.isHttpConditionEnabled = false;
        console.log(err)
        //NDListener.logBCIError(Server.TestRunIDValue, "NDHttpConditionStats", "manageAllSpecifiedConditions", " Exception while managing MonitoringLists.", err);
    }
}
    ...
    try {
        for (var i in reqTypeMap) {               //To run for loop only number of times we use map where key is only header name
            var headerName = i,
                ...
                headerId,
                headerValue
            headerValue = comingHeaderList[headerName]
            if (!headerValue){
