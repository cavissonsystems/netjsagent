...
captureProcessCoreDump.parseCommand = function(commPath='/tmp',fileName,clientSocket){
    ...
    try{
        ...
        validateCommPath(commPath,function (err){
            if(!err){
                ...
                clientSocket.write('nd_control_rep:result=Error:'+err+';\n')
            }
        })
    }catch(e){
        clientSocket.write('nd_control_rep:result=Error:Error in Parsing Command;\n')
       ...
       try{
           if( err.status == 2 && err.stdout.toString().indexOf('usage:') > -1){
               ...
               if(result.indexOf('Saved') > -1){
                   clientSocket.write('nd_control_rep:result=Success:core Dump file Created;\n')
               ...
               clientSocket.write('nd_control_rep:result=Error:Command Not found;\n')
               util.logger.error(agentSetting.currentTestRun + ' | Command Not found :')
           }
       }catch(e){
           clientSocket.write('nd_control_rep:result=Error:Execution Command is failed;\n')
