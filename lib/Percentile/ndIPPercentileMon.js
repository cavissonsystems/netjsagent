           ...
           if (keys.length != 0) {
               for (var i = 0; i < keys.length; i++) {
                   ...
                   if(data){
                       ...
                       backendDetails.prototype.resetTD(eachBackendData);
                   }
                   if(agentSetting.enableBTMonitorTrace > 0) 
                       util.logger.info(agentSetting.currentTestRun,' | Dumping 96 Record For '+eachBackendData.BackendName+' : ',data)
...
ndIPPercentileMon.generate96recordForEveryBackend = function(backendData){
    try{
        ...
        if(backendData.td && backendData.td.size() > 0){
...
ndIPPercentileMon.tDigest = function(backend,buf,offset){
    ...
    try{
       ...
       if(agentSetting.enableBciDebug > 1){
           util.logger.info(agentSetting.currentTestRun, '| IP Response Array  : '+backend.BackendName,' : ',backend.resArray);
