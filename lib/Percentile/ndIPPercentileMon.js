       ...
       if (agentSetting.autoSensorConnHandler && agentSetting.autoSensorConnHandler.client) {
           if (keys.length != 0) {
               for (var i = 0; i < keys.length; i++) {
                 ...
                 /*  if(eachBackendData.td)
                        console.log('data :',eachBackendData.BackendName,eachBackendData.td.toArray())
*/
...
ndIPPercentileMon.tDigest = function(backend,buf,offset){
    ...
    try{
        ...
        if(agentSetting.tDigestPercentileIP.sMode == 1){
            ...
            /* var newBuffer = new Buffer.allocUnsafe(backend.td.byteSize());
             backend.td.asBytes(newBuffer,0)
             fs.writeFile('/tmp/SampleIP'+counter,newBuffer,function(err){
                 if(!err)
           ...
           /*  var newBuffer = new Buffer.allocUnsafe(backend.td.smallByteSize());
             backend.td.asSmallBytes(newBuffer,0)
             fs.writeFile('/tmp/sampleIP'+counter,newBuffer,function(err){
                 if(!err)
