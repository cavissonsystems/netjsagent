...
serverMonitor.addMonitor=function(server){
    try {
        ...
        if (server && (monitors.length == 0 || !monitors.some(function (element) {
            ...
            if (host === '::' || host === '0.0.0.0' || host == undefined || host == '127.0.0.1'){
                var hostname = os.hostname();
                console.log('/etc/hostname ',hostname)
                if(!hostname)
                    host = '127.0.0.1'
                else
                    host = hostname;
            ...
            mon_server['listen'] = (host ? host :os.hostname()) + (port ? ':' + port:'');
...
function getIP(cb){
    Object.keys(iname).forEach(function(key){
        iname[key].forEach( function(value){
...
function dumpServerMonitor(mon_server){
    ...
    if(agentSetting.enableBciDebug > 5)
        util.logger.warn(agentSetting.currentTestRun + " |Dumping Node Server Monitor Record  : " ,dataStr);
}
