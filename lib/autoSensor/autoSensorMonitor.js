...
ASDataMonitor.parseStack = function(stack){
    ...
    if (stack != undefined && stack !='') {
        for (var i = 0; i < stack.length; i++) {
            if(!stack[i])continue
            if(stack[i].indexOf('netjsagent') !== -1|| stack[i].indexOf('express') !== -1||stack[i] == 'Error' || stack[i].trim().startsWith('TypeError')) {
                continue
            }
...
ASDataMonitor.sendHotSpotRecord = function(stack,duration,threadStarttime,flowpathid,methodId,threadID,timestamp) {
    ...
    try {
        var stackTrace = ASDataMonitor.parseStack(stack)
