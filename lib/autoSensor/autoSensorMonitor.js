        ...
        if (stack != undefined && stack != "") {
            for (var i = 0; i < stack.length; i++) {
                if (stack[i].indexOf("(") > -1) {
                    reportBuffer += stack[i].replace("at", "").replace("(","[").replace(")","]").trim()+"()";
                }
                else {
                    var ne = "[" + stack[i].replace("at", "").trim() + "]()"; //If No method found than appending unknown calss and unknow method ,eg:at /home/netstorm/Controller_harendra/nsecom-master/routes/checkOutAndPlaceOrder.js:431:254
                    reportBuffer += ne;
                }
                if (i + 1 !== stack.length)
                    reportBuffer += "|";
