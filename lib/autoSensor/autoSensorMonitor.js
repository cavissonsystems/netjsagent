...
var autoSensorSetting = require("./autoSensorSetting.js")														 
var reportBuffer;
...
ASDataMonitor.sendThreadSummaryReport = function(){
    ...
    try {
        //if(!agentConfReader.isTestRunning)return;
        var reportBuffer;
        ...
        if (agentConfReader.autoSensorConnHandler && agentConfReader.autoSensorConnHandler.client)
            samples.toBuffer(reportBuffer)

        asGrphData.resetCounter();
	    if(autoSensorSetting.ASTraceLevel > 0)
            util.logger.info(agentConfReader.currentTestRun,' |Dumping Thread Summary Report : ',reportBuffer)
        }
    catch(err)
    {
...
ASDataMonitor.parseStack = function(stack){
    var reportBuffer='',
        stakDepth=0;
    if (stack != undefined && stack !='') {
        for (var i = 0; i < stack.length; i++) {
            if(!stack[i])continue
            if (stack[i].indexOf('netjsagent') !== -1 || stack[i].indexOf('express') !== -1 || stack[i] == 'Error' || stack[i].trim().startsWith('TypeError') || stack[i].indexOf('async_hooks.js') != -1) {
                continue
            }
            ...
            reportBuffer += "|";
            stakDepth =stakDepth +1;
        }
    }
    reportBuffer = reportBuffer.trim().substring(0,reportBuffer.length -1)
    return {stack:reportBuffer,stakDepth:stakDepth};
}


var longStackTrace=[];
var depth=1;
function mergeStackTrace(current) {
    try {
        /*    if (!current || agentConfReader.enableHSLongStackDepth > 3 || longStackTrace.lenght > 200) {
         return;
         }*/
        if (!current || longStackTrace.lenght > 200 ||  depth > agentConfReader.enableHSLongStackDepth )
            return;

        if (current.stack) {
            longStackTrace = longStackTrace.concat(current.stack.split("\n"))
            current.stack = null;
        }
        ++depth;
        mergeStackTrace(current.parent)
    }catch(er){
        util.logger.info(agentConfReader.currentTestRun,' | Error in mergeStackTrace function of file autoSensorMonitor :',er)
    }

}

ASDataMonitor.sendHotSpotRecord = function(event,duration,threadStarttime,flowpathid,methodId,threadID,timestamp) {

    try {
        longStackTrace=[],depth=1;
        if(!Array.isArray(event)) {
            if (event.lastStack) {
                longStackTrace =longStackTrace.concat(event.lastStack.split("\n"));
                event.lastStack=null;
            }
            mergeStackTrace(event);
        }
        else
            longStackTrace=event;

        var stackTrace = ASDataMonitor.parseStack(longStackTrace)
        delete event;
        if(stackTrace.stakDepth == 0)return;
        asGrphData.setHotSpotDuration(duration, true);
        ...
        reportBuffer += agentConfReader.RECORD_SEPRATOR_COMMA;
        reportBuffer += parseInt(duration);
        reportBuffer += agentConfReader.RECORD_SEPRATOR_COMMA;
        ...
        reportBuffer += agentConfReader.RECORD_SEPRATOR_COMMA;
        reportBuffer += stackTrace.stakDepth;   //Stack depth
        reportBuffer += agentConfReader.RECORD_SEPRATOR_COMMA;
        reportBuffer += "0,0,";
        ...
        if (stackTrace.stack != undefined && stackTrace.stack != '') {
            reportBuffer += stackTrace.stack
        }
        else {
            reportBuffer += "-";
        }
        ...
        if(autoSensorSetting.ASTraceLevel > 0)
		util.logger.info(agentConfReader.currentTestRun,' |Dumping 52 Record : ',reportBuffer)   
