...
console.instrumentLoggerObj = function(obj,methods)
{
    try {
        proxy.before(obj, methods, function(obj, args, meth) {
            ...
            if(context && context.cavFlowPathId && AgentSetting.currentTestRun && AgentSetting.mapForConsoleMeths[meth]) {
                args[args.length - 1] = args[args.length - 1] + " [TOPO:" + AgentSetting.currentTestRun + ":" + AgentSetting.tier + ":" + AgentSetting.server + ":" +
                    ...
                    (context.tlFirstTierFPID ? context.tlFirstTierFPID : "") + ":" +
                    (context.ndSessionId ? context.ndSessionId : "") + ":" + (context.NVSid ? context.NVSid : 0) + ":" +
