    ...
    proxy.around(obj, 'request', function(obj, args, locals) {
        try {
           ...
           if(!flowPathId){
	        ...
	        startTime = parseInt(time.begin - agentSetting.cavEpochDiffInMills);
                        ...
                        try {
                            ...
                            proxy.callback(args, -1, function (obj, args) {
                                ...
                                try {
				                    ...
				                    takeHotspotData(tierObj.executionTime,startTime,flowpath);
                        ...
                        if(args[0] == 'response') {
                            ...
                            proxy.callback(args, -1, function (obj, args) {
                                try {
				                    ...
				                    takeHotspotData(locals.tierCall.executionTime,startTime,flowpath);
...
function takeHotspotData (endTime,startTime,flowpath) {
    try {
    if (asSettingObj.asSampleInterval > 0) {
        if (endTime > asSettingObj.threshold) {
            ...
            process.nextTick(function () {
                asManagerFile.handledHotspotData(stackTrace, endTime, startTime, flowpath.flowPathId, "", process.pid, (new Date().getTime() - agentSetting.cavEpochDiffInMills));
            });
        }
    }
    }catch (err) {
