...
module.exports = function(obj) {
    // server probe
    proxy.after(obj.Server.prototype, 'listen',
        function(obj, args,ret) {
            try{
                serverMonitor.addMonitor(ret)
            }catch (e){
                uitl.logger.warn('Error in Http Module :',e);
            }
    })
    proxy.before(obj.Server.prototype, ['on', 'addListener'],
        function(obj, args) {
            try{
                if (args[0] !== 'request') return;
                proxy.callback4flowpath(args, -1,
                    function (obj, args,local,context) {
                        try{
                            ...
                            proxy.around(res, 'end',
                                function(obj, args,local){
                                    try{
                                        local.args = args;//{}
