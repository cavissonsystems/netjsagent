    ...
    proxy.around(obj, 'request', function(obj, args, locals) {
        try {
            ...
            if(requestContext){
                ...
                flowPathId =locals.flowPathId = requestContext.flowPathId;
                flowpathObj = agentSetting.flowMap[flowPathId];
                timeInMillis =locals.timeInMillis= requestContext.timeInMillis;
                ...
                tierObj.methodId = ndMetaData.backendMeta(backendName);
                tierObj.executionTime = totalExecTime;
                tierObj.backendType = 1;
                var seqPfx = flowpathObj.seqPfx;

                if (flowpathObj.tlFirstTierFPID == undefined)
