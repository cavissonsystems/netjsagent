...
module.exports = function(obj) {
    ...
    proxy.before(obj.Server.prototype, ['on', 'addListener'], function(obj, args) {
        ...
        proxy.callback4flowpath(args, -1, function (obj, args,local,context) {
            ...
            proxy.around(res, 'end', function(obj, args,locals){
                locals.args = args;//{}
                locals.methodName = cwd+'._http_outgoing.OutgoingMessage_end'
                locals.isFirstOrlastMethod = true;
                if(agentSetting.dumpOnlyMethodExitInFP == 0)
                    methodManager.onEntry(locals)
            },function (obj, args,ret,locals) {
                if(entryPointManager.isEntryPointConfigured == 0)return;

                methodManager.onExit(agentSetting.dumpOnlyMethodExitInFP == 0 ? locals:local)
    ...
    proxy.around(obj, 'request', function(obj, args, locals) {
            ...
            if(requestContext){
                ...
                tierObj.seqId = ++flowpathObj.seqId ;
