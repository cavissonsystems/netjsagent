    ...
    proxy.around(obj, 'request', function(obj, args, locals) {
        try {
           ...
           if(!flowPathId){
            ...
            var backendName = backendRecord.generateBackendName(object.VENDOR,object.host,object.port,object.query)
		 if(agentSetting.enableBackendMonTrace > 0)
                util.logger.info(agentSetting.currentTestRun,' | Backend name is  : '+ backendName);
            //var back =  locals.backendName = object.VENDOR + '_' + object.URL;
                    ...
                    proxy.before(res, ['on', 'once','addListener'], function (obj, args) {
                        try {
                            ...
                            proxy.callback(args, -1, function (obj, args) {
								...
								if(agentSetting.enableBackendMonTrace > 0)
									util.logger.info(agentSetting.currentTestRun,' | Creating Backend Record for '+ backendName,'Time & Status : ',parseInt(tierObj.executionTime),' | ',res.statusCode);																																											 
                    ...
                    if (locals.res && locals.res.listenerCount && locals.res.listenerCount('end') === 1) {
                        if (time.done()) {
                            try {
				                ...
				                takeHotspotData(tierObj.executionTime,(+startTime + +timeInMillis),flowPathId,stackTrace);
							if(agentSetting.enableBackendMonTrace > 0)
                                util.logger.info('Creating Backend Record for '+ backendName,'Time & Status : ',parseInt(tierObj.executionTime),' | ',status);																																					  
