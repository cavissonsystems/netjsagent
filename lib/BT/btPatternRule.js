...
var patternBasedBT = require('./patternBasedBT')
        ...
        if(!agentSetting.enableStateMC) {
            ...
            if (matched == undefined) {
                ...
                if(!btObjAll)
                    ...
                    var thresholdObj = btObjAll.threshold;
                btObject = new btObj('1', 'Others', thresholdObj);
                //btObject = btObj.getOtherBT();
            }
            else if (matched.btIncluMode == 0) {
                btObject = btManager.getBtObj(matched.btName);
            }
        }else {
            var matched = patternBasedBT.getBTIDFromURL(url,req['method']);
            try {
                if (matched == undefined) {
                    var btObjAll = btManager.getBtObj("ALL");
                    if (!btObjAll)
                    ...
                    btObject = btManager.getBtObj(matched[0]);
                }
...
btPatternRule.parsePattern = function(ruleLine) {
    try {
        ...
        btConf.RegEx = new RegExp(urlPattern);
