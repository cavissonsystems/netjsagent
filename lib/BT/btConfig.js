...
btConfig.clearAllMaps = function(){
    btManager.clear();
    btGlobalRule.clearGlobalObj();
    btPatternRule.clearList();
    btConfig.resetBtId();
}
...
btConfig.parseBtThreshold = function(fileName){
    try {
        ...
        for(var j = 0; j<fileName.length; j++) {
                ...
                try {
                    ...
                    if (!btThreashold) {
                        if (DEFAULT_NAME_FOR_ALL_URI.toUpperCase() == btName.toUpperCase()) {
                            ...
                            threshold.setDefaultThreshold(thresholdObj);
                        }
                        else
                            btManager.insertBTInThresholdMap(null, btName, thresholdObj);
                    }
                    else{
                        btThreashold.threshold = thresholdObj
                        if (DEFAULT_NAME_FOR_ALL_URI.toUpperCase() == btName.toUpperCase()) {
                            btManager.insertBTInThresholdMap('0', DEFAULT_NAME_FOR_ALL_URI.toUpperCase(), thresholdObj);
                            threshold.setDefaultThreshold(thresholdObj);
                        }
            ...
            var tempBtNameMap = btManager.getBtList();
            //updating map reference in case of run time change
            if(tempBtNameMap && Object.keys(tempBtNameMap)){
                var keys = Object.keys(tempBtNameMap)
                for(var i in keys){
                    var btThresholObj = btManager.getThresholdObj(tempBtNameMap[keys[i]].btName);
                    if(!btThresholObj)
                        btThresholObj = btManager.getThresholdObj('ALL');

                    var thresholdObj = btThresholObj ? btThresholObj.threshold : threshold.getDefaultThreshold();
                    tempBtNameMap[keys[i]].threshold=thresholdObj;
                }
            }
            }
    }catch(err){
...
btConfig.parseNamingRulesForBT = function(fileName){
    try {
        ...
        //if(btConfig.isPatternBasedRulePresnt) {
            ...
            btManager.insertBT(0, 'ALL', thresholdObj, 0) //Add 0 for exclude mode, this extra field added in BT's object because we are dumping bts on basis of exclude mode.
            btManager.insertBT(1, 'Others', thresholdObj, 0)// All and Others are default bts.
    ...
    try {
        ...
        if (btConfig.isPatternBasedRulePresnt) {
            ...
            if (!btObj || !btObj.btId) {
                ...
                sb.clear();
                btName = agentSetting.encodeURI(sb,btName).toString();

                if(!btObj) {                               /*getting threshold object for current BT*/
                    var btObjAll, thresholdObj;
                    btObjAll = btManager.getBtObj("ALL");
                    if (!btObjAll)
                        thresholdObj = threshold.getDefaultThreshold();
                    else
                        thresholdObj = btObjAll.threshold;
                    btObj = btManager.insertBT(id, btName, thresholdObj,0);
                }
                else if(!btObj.btId){
...
btConfig.dumpAllBt = function(btNameMap){
    ...
    if(agentSetting.dataConnHandler) {
        if(agentSetting.isdumpBtMetaData) agentSetting.isdumpBtMetaData = false
        
        samples.add(resetBtRecord);
        util.logger.info(agentSetting.currentTestRun, "| Resetting BT, sending bt meta record : ", resetBtRecord)
    }
    for (var i in keys) {
        if (btNameMap[keys[i]].btIncluMode == 0) { //If bt is excluded no need to dump 7 record.
        ...
        btManager.btIdvsNameMap[btNameMap[keys[i]].btId] = btNameMap[keys[i]]; //Creating new btIdvsName map for storing bt object for corresponded btId.
    }
}
