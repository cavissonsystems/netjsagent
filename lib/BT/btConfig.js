...
btConfig.parseThresholdFile = function(clientMsg){
    try {
        ...
        /*if (btThresholdFile > 0)
        {
            btThresholdFileArr.push(clientMsg);
            if (clientMsg.indexOf("#end") != -1) {
                btThresholdFile = 0;
                if(btThresholdFileArr.length > 0){
                    btConfig.parseBtThreshold(btThresholdFileArr);
                }
            }
        }
...
btConfig.parseBTRuleConfigfile = function(clientMsg){
    try {
        ...
        /*if (btRuleFile > 0)
        {
            btRuleFileArr.push(clientMsg);
            if (clientMsg.indexOf("#end") != -1) {
                btRuleFile = 0;

                if(btRuleFileArr.length > 0){
                    btConfig.parseNamingRulesForBT(btRuleFileArr);
                }
            }
        }
