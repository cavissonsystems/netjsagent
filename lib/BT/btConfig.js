                ...
                try {
                    ...
                    if (DEFAULT_NAME_FOR_ALL_URI.toUpperCase() == btName.toUpperCase()) {
                       // var defaultThresholdObj = new threshold(slowThresold, verySlowThresold, dynamicThresoldMethod, dynamicSlowThresoldPct, dynamicVSlowThresoldPct);
                        ...
                        btManager.insertBT('0', DEFAULT_NAME_FOR_ALL_URI.toUpperCase(), thresholdObj);
...
btConfig.executeBTRule = function(req){
    try {
        ...
        if (btConfig.isPatternBasedRulePresnt) {
            ...
            if (!btObj || !btObj.btId) {
                id = ++btId;
                if(id >= agentSetting.maxBTCount){      //Checking for Bt limit , if btid exceeds maximum limit , then it will be count as "Other"
                    btName = 'Others'
                    id=1
                }
                if(!btObj) {                               /*getting threshold object for current BT*/
                    ...
                    btManager.insertBT(id, btName, thresholdObj);
                }
                else if(!btObj.btId){
