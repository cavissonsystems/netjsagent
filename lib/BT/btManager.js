...
//var btPercentile = require('../Percentile/btPercentileManager');
var agSet = undefined;
...
btManager.addTD = function(){

    try{
        var allBT = btManager.getbtMonCountersMap();
        var keys = Object.keys(allBT);
        if(keys.length > 0){
            for (var i in keys){
                if(allBT[keys[i]].td == null){
                    allBT[keys[i]].td = btDetails.prototype.tDigestObject();
                }
            }
            util.logger.info('| Successfully Added BT TDigest Object');
        }
    }catch (e){
        util.logger.error(agentSetting.currentTestRun,'| Error while adding TD Object',e)
    }
}

btManager.removeTD = function() {
   ...
   try{
       ...
       if(keys.length > 0){
           for (var i in keys){
               allBT[keys[i]].td = null;
               allBT[keys[i]].resArray = [];
           }
           util.logger.info('| Successfully removed BT TDigest Object.')
       }
   }catch(e){
       util.logger.error(agentSetting.currentTestRun,'| Error while Removing TD Object',e)
   }
}
...
btManager.resetMonitorCounters = function(){
    for (var i in btMonCountersMap) {
        btMonCountersMap[i].init()
    }
}
...
btManager.clearthresholdmap = function(){
    btThresholdMap={}
    threshold.setDefaultThreshold({slowThresholds:3000,verySlowThresholds:5000});
}
