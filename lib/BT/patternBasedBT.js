    ...
    try {
       ...
       // if (excludeMode == 0) {
            ...
            if (btObj == null || btObj == undefined ) {
                try {
                    var allBtObj = btManager.getBtObj("ALL");
                    if (!allBtObj) {
                        var thresholdObj = threshold.getDefaultThreshold();

                    } else {
                        var thresholdObj = allBtObj.threshold;
                    }
                }catch(ee){
                    console.log(" er    "+ee);
                }
            ...
            if (value) {
                if (m == (url.length - 1)) {
                    if (stateMachine[HTTPMETHOD][value] != undefined) {
                        return btNameIdMap[checkIfURLOnlyPatternExists(matchMethod(nextState, value, method, lastTxID), value)];
                    }
                ...
                try {
                    if (stateMachine[SKIPFLAG][nextState] || skipIdx != 0) {
                        ...
                        return btNameIdMap[checkIfURLOnlyPatternExists(matchMethod(nextState, nextState, method, lastTxID), nextState)];
                    }
                }catch(er){
                }
                return btNameIdMap[lastTxID];
