...
patternBasedBT.createAndGetPatternBasedBTDefinition = function (tempDataArray) {
    ...
    try {
        ...
        transactionId = tempDataArray[2] ? parseInt(tempDataArray[2].trim()) : NaN;
        if(transactionId !== transactionId  ) return null;

        if(tempDataArray[4] == undefined || tempDataArray[4] == '-') return null;

        matchType     =  tempDataArray[3] ? parseInt(tempDataArray[3].trim()) : NaN;
        patternBasedBTDefinition.pattern = tempDataArray[4] ? tempDataArray[4].trim() : NaN
        excludeMode   = tempDataArray[5] ? parseInt(tempDataArray[5].trim()): NaN;
        patternMode   = tempDataArray[6] ? parseInt(tempDataArray[6].trim()) : NaN;

        if(matchType !== matchType ) matchType = 0;
        if(excludeMode !== excludeMode ) excludeMode = 0;
        if(patternMode !== patternMode) patternMode = 0;

        patternBasedBTDefinition.btId = transactionId;
        patternBasedBTDefinition.patternMode = patternMode ;
      ...
      //  if(excludeMode == 0) {  //commenting this piece of code because in case of excludeMode=1  , bt is dumping in Others category.bug#26956
