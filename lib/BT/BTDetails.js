...
function BTDetails(){
    ...
    try{
        ...
        if(ag.tDigestPercentileBT && ag.tDigestPercentileBT.enable == 1 && ag.tDigestPercentileBT.k && ag.tDigestPercentileBT.delta){
            var delta = ag.tDigestPercentileBT.delta;
            if(delta > 1)
                delta = ( 1 / delta).toFixed(5)
            this.td = new TDigest(delta, ag.tDigestPercentileBT.k)
        } else {
            this.td = new TDigest(0.01,100);
            util.logger.warn(ag.currentTestRun,'| setting IP Tdigest default argument');
        }
    }catch(e){
...
BTDetails.prototype.resetTD = function(bt){
    ...
    try{
        ...
        if(bt.td){
            ...
            if(delta > 1)
                delta = ( 1 / delta).toFixed(5)
            bt.td = new TDigest(delta,ag.tDigestPercentileIP.k);
        }
    }catch(e){
...
BTDetails.prototype.createBTRecord = function (BTID, BTName, duration,cat,statusCode, slowPct, verySlowPct, reqContLength,resContLength) {
    try{
        ...
        if(duration) {
            if(ag && ag.tDigestPercentileBT && ag.tDigestPercentileBT.enable == 1 && this.td){
...
BTDetails.prototype.updateForTDAllTransation = function(bt) {

    var ag = require('../agent-setting');
    this.BTID = 0;
    this.BTName = 'AllTransactions';
    if (ag.tDigestPercentileBT.enable == 1 && this.td && bt.resArray.length > 0) {
        for(let i = 0;i<bt.resArray.length;i++){
            this.td.push(Number(bt.resArray[i]));
        }
        this.resArray =  this.resArray.concat(bt.resArray);
    }
}
