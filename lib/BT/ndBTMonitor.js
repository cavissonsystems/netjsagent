    ...
    try {
            ...
            for (var i = 0; i < keys.length; i++) {
                ...
                if (btrecordKey != undefined) {
                    if (AgentSetting.isToInstrument && AgentSetting.autoSensorConnHandler) {
                        ...
                        var singleBtData = ndBTMonitor.dump74RecordForEveryBT(eachBTData,btrecordKey)
                        overAllBTDetail.updateOverAllBTDetail(eachBTData);
                        AgentSetting.autoSensorConnHandler.write(singleBtData);
                    }
                }
            }
        if (overAllBTDetail.BTID != undefined) {
            ...
            if (AgentSetting.isToInstrument && AgentSetting.autoSensorConnHandler) {
                ...
                var overallData = ndBTMonitor.dump74RecordForEveryBT(overAllBTDetail, overAllBTDetail.BTID);

                overAllBTDetail.init();

                AgentSetting.autoSensorConnHandler.write(overallData);
            }
        }
    }
    }catch (err) {
...
ndBTMonitor.dump74RecordForEveryBT = function(eachBTData,BTID){
    ...
    btData += ' ';
    btData += '0.0';//not supported for now
    btData += ' ';
