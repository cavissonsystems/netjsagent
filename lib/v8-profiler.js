...
v8_profiler.takeHeapSnapShotOnNewConn = function(clientSocket,dataSocket,clientMsg,asyncId,command){
    ...
    try {
        ...
        var splitArr = clientMsg.split(";"), mode;
        for(var i=0; i<splitArr.length; i++){
            if(splitArr[i].indexOf("CompressMode") != -1){
                mode = splitArr[i].split("=")[1];
                break;
            }
        }
        ...
        var snapshot1 = profiler.takeSnapshot();
        var stream,chunked=0,size=0;
            gzip = zlib.createGzip();

        dataSocket.write("run_async_command_data_req:Command=" + command + ";Id=" + asyncId + ";Tier=" + agentSetting.tier + ";Server=" + agentSetting.server + ";Instance=" + agentSetting.instance + ";Size=-1;CompressMode="+mode+"\n")
        ...
        if(mode == 1) {
            var zstream = stream.pipe(zlib.createGzip())
            var socketStream = zstream.pipe(dataSocket.client,{end:false})
            ...
            stream.pipe(dataSocket.client,{end:false})
            stream.on('end',function(){
                dataSocket.write('Heapdump:Result=OK;')
                dataSocket.closeConnection();
                ut.logger.info(agentSetting.currentTestRun + "| End event Reached For heapsnapshot stream")
       ...
       /* stream = snapshot1.export()
            ...
            .on('data', function (chunk) {
                size += chunk.length
                ++chunked
                var flag = (dataSocket.client.write(chunk), function (err) {
                })           //For back pressuring we are using data event ,not pipe
                if (!flag) {
                    stream.pause();
                    ut.logger.info(agentSetting.currentTestRun + "There will be no additional data for 0.5 second.");
            ...
            .on('end', function (chunk) {
                snapshot1.delete();
                dataSocket.client.write("Heapdump:Result=OK;")
                dataSocket.closeConnection();
              //  nc.closeConnection();
                ut.logger.info(agentSetting.currentTestRun + " | Heapdump:Result=OK;")
                ut.logger.info(agentSetting.currentTestRun + " | Total dumped data : ", size,"chunked : ",chunked)
            })*/
        }
        catch (e){
