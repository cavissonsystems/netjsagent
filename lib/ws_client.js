...
var connectConnListener = function() {
    try {
        ...
        if(!controlMessageHandler) {
            controlMessageHandler = new MessageHandler(ws_client,'ws');
        }
        ...
        controlMessageHandler.setClientSocket(ws_client,'ws')
...
ws_client._connect = function()
{
    try{
        ...
        ndcPort = serverConfig.currentActiveServer.ndcPort;
        if((serverConfig.currentActiveServer.ndcHost == undefined) || (serverConfig.currentActiveServer.ndcPort == undefined))
            connectToServer();
        else {
            var url = 'ws://'+serverConfig.currentActiveServer.ndcHost+':'+serverConfig.currentActiveServer.ndcPort+'/' ;
