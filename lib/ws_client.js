...
var closeConnListener = function(err) {
    try {
        ...
        connectToServer(serverConfig.sleepInterval);
    }
...
ws_client.connectToServer = function(){
    ...
    //ws_client._connect();
    connectToServer(agentSetting.connectInterval);
};

function connectToServer(waitTime)
{
    ...
    util.logger.warn(agentSetting.currentTestRun + ' | @ws_client.js->connectToServer() please Wait for '+waitTime/1000+' Seconds');
    ...
    socket_timeout = setTimeout(function () {
        try {
            ...
            util.logger.info(agentSetting.currentTestRun+" | Timer for retrying control connectoion expired. trying to connect with Host : "+ndcHost+" ,Port : "+ndcPort, "Type : ",serverConfig.currentActiveServer.type);
            ws_client._connect();
        }
        catch(err){util.logger.warn(agentSetting.currentTestRun+" | Error in retrying" + err);}
    },waitTime);
}
ws_client.urlbuilder=function(){
    var os = require('os');
    try {
        var tmpurl = '';
        if(agentSetting.endpointPrefix === undefined || agentSetting.endpointPrefix == null || agentSetting.endpointPrefix == 0 || agentSetting.endpointPrefix.length <= 0)
            tmpurl = tmpurl + '/Collector';
        else
            tmpurl = tmpurl+'/'+agentSetting.endpointPrefix;

        tmpurl = tmpurl + '/nd';        //because every time it will be nd in agent case

        if(agentSetting.appname === undefined || agentSetting.appname == null || agentSetting.appname == 0 || agentSetting.appname.length <= 0)
            tmpurl = tmpurl+'/Default';
        else
            tmpurl = tmpurl+'/'+agentSetting.appname;

        if(agentSetting.tier === undefined || agentSetting.tier == null || agentSetting.tier == 0 || agentSetting.tier.length <= 0)
            tmpurl = tmpurl+'/NA';
        else
            tmpurl = tmpurl+'/'+agentSetting.tier;

        if(agentSetting.server === undefined || agentSetting.server == null || agentSetting.tier == 0 || agentSetting.server.length <= 0)
            tmpurl=tmpurl+'/'+os.hostname();
        else
            tmpurl=tmpurl+'/'+agentSetting.server;

        tmpurl=tmpurl+'?';          //Url created , Query Parameter begins

        if(agentSetting.tierAttributes === undefined || agentSetting.tierAttributes == null || agentSetting.tierAttributes == 0 || agentSetting.tierAttributes.length <= 0)
            tmpurl=tmpurl+'tierAttributes=NA&';
        else
            tmpurl=tmpurl+'tierAttributes='+agentSetting.tierAttributes+'&';
    }
    catch(err){
        util.logger.warn(agentSetting.currentTestRun+" | URL builder failed to create url" + err);
    }
    return tmpurl;
}
...
ws_client._connect = function()
{
    try{
        ...
        if((serverConfig.currentActiveServer.ndcHost == undefined) || (currProto.port == undefined))
            ...
            var url = currProto.protocol+'://'+serverConfig.currentActiveServer.ndcHost+':'+currProto.port+ws_client.urlbuilder();
            var options={}
...
ws_client.closeConn = function(){
    try{
        ...
        util.logger.error(agentSetting.currentTestRun + ' | Error in ws_client closeConn()  '+e);
    }
}
...
ws_client.getLocalPort = function(){
    try {
        if (!client && !client_socket)return;
        return client._socket.address() ? client._socket.address().port : undefined
    }catch(err){util.logger.warn(agentSetting.currentTestRun + ' | Failed to getLocalPort ');return}
}
...
ws_client.getLocalHost = function(){
    ...
    }catch(err){util.logger.warn(agentSetting.currentTestRun + ' | Failed to getLocalHost ');return}
}
