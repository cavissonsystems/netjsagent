...
MyFormatter.prototype.onEntry = function(args) {
    ...
    try{
        ...
        if (dirName == ".")
            dirName = process.cwd().substring(process.cwd().lastIndexOf(path.sep)+1,process.cwd().length);
        ...
        util.logger.warn(agentSetting.currentTestRun+" | Error in dumping entry data" + err);
    }
};
...
*/
        ...
        var dirName = path.dirname(args.entryData.file);
        var baseName = path.basename(args.entryData.file, '.js');
        ...
        //var methodData = dirName + "." + baseName + '.' + args.name + '(' + functionArguments + ')_' + args.line;
        var methodData = dirName + "." + baseName + '.' + args.entryData.name ;
        ...
        {
            if(flowpath.calls[i].methodId == ndMethodMetaData.getValue(methodData)) {
        ...
        process.nextTick(function(){
            try {
                var mthObj = ndMethodMonitor.isMethodInCurrentMonitoringList(methodData);
                if (mthObj) {
                    var aliasName;
                    aliasName = mthObj;
                    ndMethodMonitor.updateMMCounters(methodData, methodObj.methodId, endTime, aliasName)
                }
            }
...
//        }
    ...
    {
        util.logger.warn(agentSetting.currentTestRun+" | Error in dumping exit data:" + err);
    }
};
