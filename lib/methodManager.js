...
MyFormatter.prototype.onEntry = function(args) {
    ...
    var methodId, last_arg

    last_arg = agentSetting.corelateEventCallback >0 ? args.args && args.args[args.args.length -1 ] && args.args[args.args.length -1][0] && args.args[args.args.length -1 ][0].toString().indexOf('Delay') !== -1 && args.args[args.args.length -1][0] : undefined
    if(last_arg)delete args.args[args.args.length -1]
    ...
    if(eventName) args.parentId = context.seqId;

    if(agentSetting.corelateEventCallback >0 && (last_arg || eventName )) {
        var eventInfo, delay,argsName; ;
        if(last_arg) {
        ...
        //to do dump 6 record
        var headerId = NDHttpCaptureSettings.getIDAndDumpHttpMetaRecord('asyncEvent')
