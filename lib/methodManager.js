...
MyFormatter.prototype.onEntry = function(args) {
    ...
    try{
        ...
        var  flowpath = agentSetting.flowMap[requestedObj['cavFlowPathId']];
        if(!flowpath)
            return;
        ...
        if(!args.line)
            ...
            lineNum = ';'+(args.line);
            //args.line = ';'+(args.line-1);

		var methodData = dirName + "." + baseName + '.' + args.name + lineNum ;
        ...
        if(flowpath.nonServiceMethodDepth > agentSetting.bciMaxNonServiceMethodsPerFP) {
            util.logger.error(agentSetting.currentTestRun, '| Not dumping method entry record for methodId : ',args.methodId,'because nonServiceMethodDepthis ',
        ...
        try {
            process.nextTick(function () {
                try {
                    ...
                    flowpath.category = category.getCategory(flowpath.statusCode, respTime, btObj.threshold, bt_Data.dynamicSlowThresold, bt_Data.dynamicVSlowThresold);// this function responsible for calculate slow and vslow flowpaths on basis of threshold values
                    btManager.createAndUpdateBTRecord(btObj.btId, btObj.btName, respTime, flowpath.category, flowpath.statusCode);

                    if (context.cavIncludeFp || (!(context.cavIncludeFp) && flowpath.category > 10)) {
...
MyFormatter.prototype.onExit = function(args)
{
    try {
        ...
        args.methObj.duration = endTime;
        if (Number(endTime) < agentSetting.excludeMethodOnRespTime) {
            args.methObj.exclude = true;
            //
            --args.flowPathObj.nonServiceMethodDepth;
            return;
        }
        ...
        if(args.flowPathObj.nonServiceMethodDepth > agentSetting.bciMaxNonServiceMethodsPerFP) {
            util.logger.error(agentSetting.currentTestRun, '| Not dumping method exit record for methodId : ',methodObj.methodId,'because nonServiceMethodDepthis ',
                args.flowPathObj.nonServiceMethodDepth, ' is greater then', agentSetting.bciMaxNonServiceMethodsPerFP,'.FPID is :',args.flowPathObj.flowPathId);
            args.flowPathObj.nonServiceMethodDepth = --args.flowPathObj.nonServiceMethodDepth;
            return;
        }
                ...
                process.nextTick(function () {
                    try {
                        if(asSettingObj.ASTraceLevel > 0)
                            util.logger.info(agentSetting.currentTestRun,' | Handling hotspot for methodId :',methodObj.methodId)
