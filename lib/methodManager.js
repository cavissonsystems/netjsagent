...
MyFormatter.prototype.onEntry = function(args) {
    ...
    var flowpath=agentSetting.isTestRunning && (context= agentSetting.getContextObj()) && context.cavFlowPathId && agentSetting.flowMap[context.cavFlowPathId]
    ...
    var methodId, startUpTime;
    methodId=  ndMethodMetaData.getValue(args.methodName,args);
    args.methObj = new methodCall (methodId,'_0_',new Date().getTime()-context.fpTimeWithoutCavepoch);          //Creating Obj for every method entry
        ...
        try {
            process.nextTick(function () {
                try {
                    ...
                    btManager.createAndUpdateBTRecord(btObj.btId, btObj.btName, respTime, flowpath.category, flowpath.statusCode,bt_Data.dynamicSlowThresold, bt_Data.dynamicVSlowThresold, reqSize,resSize);
...
MyFormatter.prototype.onExit = function(args) {
    ...
    if(!agentSetting.isTestRunning || !flowPathObj) return;
