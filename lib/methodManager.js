...
MyFormatter.prototype.onEntry = function(args) {
    ...
    try{
        ...
        agentSetting.seqId = ++agentSetting.seqId;
        ...
        methodObj.event =  '_0_' ;
        args.cavMthStartTime = methodObj.startUpTime = (new Date().getTime()-agentSetting.cavEpochDiffInMills)-requestedObj.timeInMillis; ;
...
MyFormatter.prototype.onCompleteFlowPath = function(req,res) {
    ...
    //var respTimeAccCav = resp_current_time - (agentSetting.cavEpochDiffInMills);
    var respTime = (new Date().getTime()-agentSetting.cavEpochDiffInMills)-req.timeInMillis; ;
...
MyFormatter.prototype.onExit = function(args)
{
    try {
        ...
        var endTime = ((new Date().getTime() - agentSetting.cavEpochDiffInMills)  - args.flowPathObj.timeInMillis) - args.cavMthStartTime;
        ...
        if(asSettingObj.asSampleInterval > 0 ) {
            if (endTime > asSettingObj.thresholdValue) {
                ...
                process.nextTick(function () {
                    try {
                        asManagerFile.handledHotspotData(stackTrace, endTime, args.cavMthStartTime,
                            args.flowPathObj.flowPathId,
                            methodObj.methodId, threadID, (new Date().getTime() - agentSetting.cavEpochDiffInMills));
                    }
                    catch (err) {
                        console.log(" threshold value " + asSettingObj.thresholdValue + " endTime " + endTime + " thread id is " + threadID);
