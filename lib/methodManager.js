...
MyFormatter.prototype.onEntry = function(args) {
    ...
    var last
    last = agentSetting.instrumentNodeEvents && args.args && args.args[args.args.length -1 ] && args.args[args.args.length -1][0] && args.args[args.args.length -1 ][0].indexOf(';delay') !== -1 && args.args[args.args.length -1][0]
    if(last) {
        delete args.args[args.args.length -1]
        var eventInfo = last.split(':'),
            delay,argsName;
        delay = startUpTime - eventInfo[1]
        argsName = eventInfo[0]+'_'+delay;
        //to do dump 6 record
