...
function BackendDetails(){
    ...
    //this.errorRate = 0;
    this.td = BackendDetails.prototype.tDigestObject();
    //debugging purpose
    this.resArray = []
}
...
BackendDetails.prototype.tDigestObject = function() {
    try{
        ...
        if(ag.tDigestPercentileIP && ag.tDigestPercentileIP.enable == 1 && ag.tDigestPercentileIP.delta && ag.tDigestPercentileIP.k){
            ...
            return new TDigest(delta ,ag.tDigestPercentileIP.k)
        }
        else {
            return null;
        }
    }catch(e){
        util.logger.error( 'Error in constructing TD Object : ',e)
    }
}
...
BackendDetails.prototype.commonCalculation = function(status,duration,netReqDelay,netResDelay){
    try {
        ...
        if(duration && ag && ag.tDigestPercentileIP && ag.tDigestPercentileIP.enable == 1 && this.td){
