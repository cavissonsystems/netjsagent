...
backendRecord.addTD = function() {
    try{
        var allIP = backendRecord.getBackendRecordMap();
        var keys = Object.keys(allIP);
        if(keys.length > 0){
            for(let i in keys){
                if(allIP[keys[i]].td == null){
                    allIP[keys[i]].td = backendDetails.prototype.tDigestObject();
                }
            }
            util.logger.info(agentSetting.currentTestRun,'| Successfully Added '+keys.length+' Backend TDigest Object.')
        }
    }catch(e){
...
backendRecord.removeTD = function() {
    try{
        var allIP = backendRecord.getBackendRecordMap();
        var keys = Object.keys(allIP);
        if(keys.length > 0){
            for(let i in keys){
                allIP[keys[i]].td = null;
                allIP[keys[i]].resArray = [];
            }
            util.logger.info(agentSetting.currentTestRun,'| Successfully removed '+keys.length+' Backend TDigest Object.')
        }
    }catch(e){
...
backendRecord.getBackendRecordMap = function() {
    return agentSetting.backendRecordMap;
}
