    ...
    try {
        ...
        if (list.length) {
            if (currKeyword == "ndMethodMonFile") {
                ...
                instrProfileParser.checkAndInstrumentLoggerObject();
            }
...
instrProfileParser.checkAndInstrumentLoggerObject = function() {
    if (agentSetting.enableUpdateLogMsgForNF) {
        if (entryPonitManager.entryPointMap['CONSOLE'] && entryPonitManager.entryPointMap['CONSOLE'].enable == 1) {
            var instrumentConsole = require('./nodetime/lib/probes/console');
            agentSetting.mapForConsoleMeths = {};
            var methArr = entryPonitManager.entryPointMap['CONSOLE'].method.split(",");
            for (var i in methArr)
                agentSetting.mapForConsoleMeths[methArr[i]] = true;
            if (!console._isInstrumented) {
                instrumentConsole.instrumentLoggerObj(console, methArr);
                console._isInstrumented = true;
            }
        } else {
            agentSetting.mapForConsoleMeths = {};
        }
        if (entryPonitManager.entryPointMap['WINSTON'] && entryPonitManager.entryPointMap['WINSTON'].enable == 1 && entryPonitManager.entryPointMap['WINSTON'].method) {
            agentSetting.mapForWinstonLogMeths = {};
            var methArr = entryPonitManager.entryPointMap['WINSTON'].method.split(",");
            for (var i in methArr)
                agentSetting.mapForWinstonLogMeths[methArr[i]] = true;
        } else {
            agentSetting.mapForWinstonLogMeths = {};
        }
    }
...
instrProfileParser.isValidExpectedFileMetaData = function(expectedFileMetaData,clientSocket,makeDataAutoConnection,callback){

    try{
        if(typeof expectedFileMetaData == 'object' ){
            console.log('Size :',expectedFileMetaData.size > 0,typeof expectedFileMetaData.type);
        ...
        util.logger.error(agentSetting.currentTestRun,' | ')
    }
