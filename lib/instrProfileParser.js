...
instrProfileParser.processInstrFile = function (clientMsg,clientSocket,makeDataAutoConnection) {
    ...
    try {
        if (instPrfCount > 0 || list.length) {
           ...
           /* util.logger.info("agentSetting.invalidFileFormatMsg : ",agentSetting.invalidFileFormatMsg,"runTimeChange : ",runTimeChange)
            if(agentSetting.invalidFileFormatMsg) {
		        if(clientMsg.indexOf('nd_control') == -1 &&  clientMsg.indexOf('nd_ctrl_') == -1) {
                    agentSetting.invalidFileFormatMsg = false
                    //runTimeChange=false;
                	clientSocket.write("nd_control_rep:action=start_instrumentation;status=" + agentSetting.status + ";result=Ok;" + '\n')
                    agentSetting.isToInstrument = true;
                    makeDataAutoConnection();
