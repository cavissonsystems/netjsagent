...
function checkFileBasedKeywordAndMakeConn(clientSocket,makeDataAutoConnection){
    if(fileBasedKeyword.length && multiFileKeywordCount < fileBasedKeyword.length ) {
        ...
        getFileContent(fileBasedKeyword[multiFileKeywordCount].value, clientSocket, currKeyword,makeDataAutoConnection)
    }
    else {
        ...
        else if(runTimeChange){                 //runtime change will be true, if it is applied but file is same then we have to handle that case
            runTimeChange=false;
...
function getFileContent(allFields,clientSocket,keywordName,makeDataAutoConnection)
{
    ...
    try
    {
        ...
        instrProfileParser.compareAndGetFileContents(oldModel,newFileName,clientSocket,makeDataAutoConnection);
...
instrProfileParser.compareAndGetFileContents = function(oldModel,newModel, clientSocket,makeDataAutoConnection) {
    try {
        var isChanged = false;
        if (!newModel) {
        ...
        runTimeChange = agentSetting.runTimeChange=false
    ...
    try {
        ...
        for (var i = 0; i < allFields.length; i++) {
            if (allFields[i].toString().startsWith("InstrProfile=") || allFields[i].toString().indexOf("InstrProfile=/") != -1 || allFields[i].toString().startsWith("instrProfile=") || allFields[i].toString().indexOf("instrProfile=/") != -1) {
                ...
                fileBasedKeyword.push({keywordName:keywordName,value:allFields[i]})
            }
