...
instrProfileParser.processInstrFileList = function (clientMsg,clientSocket) {
    ...
    try {
        ...
        for (var i = 0; i < allFields.length; i++) {
            ...
            else if(allFields[i].trim().startsWith('ndBackendNamingRulesFile') && (clientMsg.indexOf("ndBackendNamingRulesFile=NA;") == -1)) {
                keywordName = 'ndBackendNamingRulesFile';
                fileBasedKeyword.push(keywordName)
            }
            else if(allFields[i].trim().startsWith("HTTPStatsCondCfg=") && (clientMsg.indexOf("HTTPStatsCondCfg=NA;") == -1)){
