...
ndMethodMonitor.makeMM = function(data)
{
    try {
        ...
        return '61,' + AgentSetting.vectorPrefixID + data.methodId + ':' + vectorPrefix + data.aliasname + '|' + data.cumCount + ' ' + data.rate + ' ' + data.avgDuration + ' ' + data.minDuration + ' ' + data.maxDuration + ' ' + data.invocationCount + '\n';
    }
    catch(err)
    {util.logger.warn(err)}
}
