...
ndMetaDataRecoveryProcess.parseCurrentRequestMessage = function(clientMsg, clientSocket){
    ...
    try {
        ...
        if (allFieldsOfClientMessage.length > 6) {
            ...
            //Send all metadata
            destination = 0;
            ndMetaDataRecoveryProcess.handleAllMetaDataSending(clientSocket,false);
        }
        else {
...
ndMetaDataRecoveryProcess.handleAllMetaDataSending = function(clientSocket,sendMetaDataOnStartInstr){
    if(sendMetaDataOnStartInstr)                //Changing destination to "1" (1= Send info on data Conection),To send meta data on starting of test
        destination=1;

    ndMetaDataRecoveryProcess.dump11Record("All");
    ndMetaDataRecoveryProcess.sendCachedDataToNewClient(5, clientSocket);
    ndMetaDataRecoveryProcess.sendCachedDataToNewClient(6, clientSocket);
...
ndMetaDataRecoveryProcess.handleCurrRecord = function(clientMsg, typeOfRecordField, clientSocket){
    ...
    else if(typeOfRecordField.startsWith("21") || typeOfRecordField.startsWith("23"))
    {
...
ndMetaDataRecoveryProcess.sendCachedSpecificSingle21Record = function(rcType, index, clientSocket){
    ...
    if(data) {
        ...
        sb = new StringBuffer();
        sb.clear();
        record = rcType + ",0," + index + "," + ndSQLProcessor.encodeQuery(sb, data.toUpperCase()) + '\n';
...
ndMetaDataRecoveryProcess.sendCached21DataToNewClient = function(rcType, index, clientSocket){
    for(var i = 1; i<= index; i++){
        ...
        if(val) {
            ...
            sb = new StringBuffer();
            sb.clear();
            record = rcType + ",0," + i + "," + ndSQLProcessor.encodeQuery(sb, val.toUpperCase()) + '\n';
            ndMetaDataRecoveryProcess.sendData(record, clientSocket);
        }
    }
...
ndMetaDataRecoveryProcess.sendMultipleCachedDataFor6Rec = function(j, index, clientSocket){
    var headerMap = NDHttpCaptureSettings.handleMetadataRecovery()
    var keys = Object.keys(headerMap);
    for (var i in keys){
        var id = headerMap[keys[i]]
        if(id > index)
            break;
        var str = '6,'+ keys[i] +','+ headerMap[keys[i]] + '\n';
        ndMetaDataRecoveryProcess.sendData(str, clientSocket);
    }
};

ndMetaDataRecoveryProcess.sendCachedDataFor6RecToNewClient = function(j, clientSocket){
    var headermap = NDHttpCaptureSettings.handleMetadataRecovery()
    for(var i in headermap){
        var str = '6,'+ i +','+ headermap[i] + '\n';;
        ndMetaDataRecoveryProcess.sendData(str, clientSocket);
    }
};

ndMetaDataRecoveryProcess.sendCachedSpecificSingle6Record = function(rcType, index, clientSocket){
    var headerMap = NDHttpCaptureSettings.handleMetadataRecovery()
    var isRecordFound = false;
    for(var i in headerMap){
        if(headerMap[i] == index){
            var record = rcType + "," + i + "," + headerMap[i] + '\n';
            ndMetaDataRecoveryProcess.sendData(record, clientSocket);
            isRecordFound = true;
        }
    }
    if(!isRecordFound) {
        util.logger.warn(agentSetting.currentTestRun + " | No cached element found in " + 6 + " record, for specified index : " + index);
    }
};
...
ndMetaDataRecoveryProcess.collectAndSendOnlyCached6Records = function(index, isSpecific, isMultiple, clientSocket) {
    try {
        if (index != 0) {
            if (isSpecific && !isMultiple)
                ndMetaDataRecoveryProcess.sendCachedSpecificSingle6Record(6, index, clientSocket);
            else
                ndMetaDataRecoveryProcess.sendMultipleCachedDataFor6Rec(6, index, clientSocket);
        }
        else {
            ndMetaDataRecoveryProcess.sendCachedDataFor6RecToNewClient(6, clientSocket);
        }
    }catch(err){
        util.logger.warn(err);
    }
...
ndMetaDataRecoveryProcess.sendCachedMultipleDataToNewClient = function(rcType, index, clientSocket) {
    for (var i = 1; i <= index; i++) {
        var record = "na";
        var val = ndMethodMetaData.getMethodMonitorName(i);
        if(val) {
            record = rcType + "," + val + "," + i + '\n';
            ndMetaDataRecoveryProcess.sendData(record, clientSocket);
        }
...
ndMetaDataRecoveryProcess.sendCachedDataToNewClient = function(rcType, clientSocket){
    ...
    if(rcType == '5') {
        ...
        this.sendCachedDataFor6RecToNewClient(6,clientSocket)
    }else if(rcType == '7'){
...
ndMetaDataRecoveryProcess.sendCachedSpecificSingleRecord = function(rcType, index, clientSocket){
    var record = "na";
    var data = ndMethodMetaData.getMethodMonitorName(index);
    if(data) {
...
ndMetaDataRecoveryProcess.sendData = function(record, clientSocket){
    ...
    util.logger.info(agentSetting.currentTestRun +" | " + "Meta Data Recovery process, dumping record on client message with value : ",record);
    if(destination == 0)
