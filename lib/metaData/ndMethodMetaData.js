...
ndMethodMetaData.getValue = function(key,args){              //map[(data),(id)]
    ...
    if(!id)
        ...
        args.dumpedMethodName = methodMonitorMap[id]
...
ndMethodMetaData.backendMeta = function(methodName){
    try{
        ...
        if(!id){
            ...
            var encodedIPName = agentSetting.escapeHTMLForBackend(methodName)
            var methodMetaData = '5,' + encodedIPName + ',' + methodId;
            samples.add(methodMetaData + "\n");
            methodMonitorMap[methodId] = encodedIPName;
        }
        return id ;
    } catch (err) {
    ...
    try{
        ...
        if (agentSetting.isToInstrument) {
                ...
                for (k in args.args) {
                    ...
                    if(obj) {
                        if (obj.constructor && obj.constructor.name)
                            functionArguments = functionArguments + "" + obj.constructor.name + ";";
                        else
                            functionArguments = functionArguments + "" + typeof obj + ";";
                    }else
                        functionArguments = functionArguments + 'error;'
                }
            }
            var methdoName_line;
            ...
            **/
            ...
            args.dumpedMethodName = methodName
            samples.add(methodMetaData + "\n");
            }
        }
        return methodId;
