...
ndExceptionCaptureSettings.parseExceptionFilteringList = function (exceptionFilterList) {
    try {
        ...
        if (exceptionFilterList.equalsIgnoreCase("NA")) {
        ...
        for (var i = 0; i < allFilteringExceptions.length; i++) {
            ...
            if (!ndExceptionMonitor.exceptionFilterList.indexOf(currElement)>-1) {        //todo check indexOf condition
                ndExceptionMonitor.exceptionFilterList.add(currElement);
            }
        }
    }
    catch (err) {
        util.logger.warn(agentConfReader.currentTestRun + " | ndExceptionCaptureSettings", "parseExceptionFilteringList", "Exception occured in parsing filteringlist.", err);
    }
    //TO DO=====================================================================
