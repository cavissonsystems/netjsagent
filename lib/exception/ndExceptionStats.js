...
var excMetaRecoverMap = new Object();
...
ndExceptionStats.handleExceptionParameters = function (compArgs, backendId, seqNumber, type, subtype) {
    ...
    try {
        ...
        if (ndExceptionCaptureSettings.getExceptionCaptureMode() > 0)
        {
...
ndExceptionStats.dumpExceptionMetaRecord = function (recordId, exceptionRecord, exceptionStatId) {
    try {
        ...
        var record = sb.toString() + "\n";
        excMetaRecoverMap[recordId+"_"+exceptionStatId] = record;
        samples.add(record);
    }
    catch (err)
    {
...
ndExceptionStats.getAll = function(recordId){
    var data = [];
    for(var i in excMetaRecoverMap){
        if(i.startsWith(recordId)){
            var meta = excMetaRecoverMap[i];
            data.push(meta)
        }
    }
    return data;
...
ndExceptionStats.getKey = function(recordId,uniqueId){
    var meta = excMetaRecoverMap[recordId+"_"+uniqueId];
    return meta;
};

ndExceptionStats.getLength = function(recordId){
    var count = 0;
    for(var i in excMetaRecoverMap) {
        if (i.startsWith(recordId)) {
            count++;
...
ndExceptionStats.parseStack = function(stackTraceArr){
    ...
    if (stackTraceArr) {
        ...
        var reportBuffer='';
        for(i in stack){
            if(stack[i].indexOf('netjsagent') !== -1 ||stack[i].trim().startsWith('Error') || stack[i].trim().startsWith('TypeError')) {
                continue
            }
