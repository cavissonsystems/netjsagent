...
ndExceptionMonitor.exceptionFilterList = new Object();
        ...
        try {
            ...
            for(var j in exceptionMonList) {
                ...
                if (allFields.length > 1) {
                    exceptionClassName = allFields[0].toString();
                    if (null == exceptionClassName)
                        continue;
                    displayName = allFields[1].toString();
                }
                else
                    continue;

                var key = Object.keys(ndExceptionMonitor.currentMonitoringExceptionList);
                var val = new Object();
                for (var i = 0; i < key.length; i++) {
                    val = ndExceptionMonitor.currentMonitoringExceptionList[key[i]];
                }

                if (key.indexOf(exceptionClassName) == -1) {
                    if (!displayName) {
                        ndExceptionMonitor.currentMonitoringExceptionList[exceptionClassName] = exceptionClassName;
                        continue;
                    } else if (val.length >=0) {
                        if(val.indexOf(displayName) > -1) {
                            continue;
                        }
                    }
