        ...
        if(AgentSetting.autoSensorConnHandler && AgentSetting.autoSensorConnHandler.client) {
            ...
            if (keys.length) {
                for (var i in keys) {
                    ...
                    var data = ndExceptionMonitor.createExceptionData(AgentSetting.vectorPrefixID, eId, AgentSetting.vectorPrefix, aliasName, cumCount, rate);
                ...
                var data = ndExceptionMonitor.createExceptionData(AgentSetting.vectorPrefixID, 0, AgentSetting.vectorPrefix, 'OverAll', overall_cumcount, overall_rate);
                AgentSetting.autoSensorConnHandler.write(data);
...
ndExceptionMonitor.resetMonitorCounter = function () {
    for(var i in tlMonitoredExceptionDataObjList){
        tlMonitoredExceptionDataObjList[i].initOnStartInstr()
    }
}
