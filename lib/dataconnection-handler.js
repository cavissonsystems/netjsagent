...
dataConnHandler.prototype.createDataConn = function(){
    try {
        ...
        this.client.on('close', function(err) {
            ...
            util.logger.warn(agent.currentTestRun+" | Data connection], Received socket close event from Host : "+agent.getNDCHost()+" ,Port="+agent.getPort() +", Error :  "+err,"IsClient Writable",self.client.writable);
        });
