...
AgentSetting.getData = function (filename) {
    ...
    try {
        ...
        if((AgentSetting.ndcHost == null) || (AgentSetting.ndcHost == undefined) || (AgentSetting.ndcHost == 0) || AgentSetting.ndcHost.trim() == '') {
            AgentSetting.ndcHost = defaultNdcHost;
            util.logger.error(AgentSetting.currentTestRun + " | ndcHost is not present in ndsetting file , so setting it to default : " + filename);
        }
        if((AgentSetting.ndcPort == null) || (AgentSetting.ndcPort == undefined) || (AgentSetting.ndcPort == 0) || AgentSetting.ndcPort.trim() == '') {
            AgentSetting.ndcPort = defaultNdcPort;
