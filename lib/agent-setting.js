...
AgentSetting.cavEpochDiffInMills =0 ;
...
var envSettinfFile = path.join(AgentSetting.ndHome,'/ndsettings.conf')
...
AgentSetting.ndMonitorInterval = 10000;
...
AgentSetting.THREAD_PRIORITY = "5";
...
{
    try {
        if (fs.existsSync(envSettinfFile) && util.canWrite(envSettinfFile)) {
            util.logger.info("NDHome is set : "+AgentSetting.ndSettingFile);
            ...
            util.logger.info("NDHome is not set , so taking default path: "+AgentSetting.ndSettingFile);
            AgentSetting.getData(AgentSetting.ndSettingFile);
        }
            ...
            util.logger.info("path acc to project : "+AgentSetting.ndSettingFile);
            AgentSetting.getData(AgentSetting.ndSettingFile);
        }
        else{
            util.logger.error("No NdSetting file is prsent on server");
        }
