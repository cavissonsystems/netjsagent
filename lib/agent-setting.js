...
AgentSetting.httpReqCapturingSettings = new NDHttpReqRespCaptureSettings()
AgentSetting.httpResCapturingSettings = new NDHttpReqRespCaptureSettings()
...
AgentSetting.initAllMap = function(logMode)
{
    try {
        ...
        if(!logMode)
            logMode='FILE'
        AgentSetting.BCILoggingMode=logMode
    }
    catch(err){util.logger.warn(err)}
...
AgentSetting.getData = function (filename) {
    ...
    try {
        ...
        if(excludeMethodOnRespTime == null || excludeMethodOnRespTime == 0)
            excludeMethodOnRespTime = 0
        ...
        AgentSetting.dynamicThreshold = properties.get('enableDynamicThreshold');
        ...
        util.logger.info(AgentSetting.currentTestRun + " | Mode : " + AgentSetting.excludeMethodOnRespTime);
        util.logger.info(AgentSetting.currentTestRun + " | Mode : " + AgentSetting.dynamicThreshold);
