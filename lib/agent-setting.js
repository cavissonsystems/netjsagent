...
AgentSetting.readAgentConfigInEnv = function(){
    ...
    {   //@ the end this if is returning true and else is returning false
        ...
        if(isEmpty(AgentSetting.appname)){
            ...
            util.logger.error(AgentSetting.currentTestRun + " | appName is not present in Environment variable , so setting it to default : ");
        }
        if(isEmpty(AgentSetting.ndcHost)){
            AgentSetting.ndcHost = '';
            util.logger.error(AgentSetting.currentTestRun + " | ndcHost is not present in Environment variable , so setting it to default : ");
        }
        if(isEmpty(AgentSetting.ndcPort)) {
            AgentSetting.ndcPort = defaultNdcPort;
            util.logger.error(AgentSetting.currentTestRun + " | ndcPort is not present in Environment variable , so setting it to default: ");
        }
...
AgentSetting.getData = function () {
    ...
    try {
        ...
        if(isEmpty(AgentSetting.appname)) {
            AgentSetting.ndcHost = '';
            util.logger.error(AgentSetting.currentTestRun + " | appName is not present in ndsetting file , so setting it to default : " + filename);
        }
        if(isEmpty(AgentSetting.ndcHost)) {
            AgentSetting.ndcHost = '';
