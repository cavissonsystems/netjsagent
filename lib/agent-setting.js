...
AgentSetting.connectInterval = 30000;
var envSettinfFile ;
...
AgentSetting.settingFileMode = "shared";
...
AgentSetting.isAgentConfigDefinedInEnv = function(){
    ...
    {
        util.logger.info(AgentSetting.currentTestRun+" | Environment variable is Available ")
        return true;
    }
    else
    {
        util.logger.info(AgentSetting.currentTestRun+" | Environment variable Not Found  ")
        return false;
    }
}

AgentSetting.readAgentConfigInEnv = function(){

    if(this.isAgentConfigDefinedInEnv())
    {   //@ the end this if is returning true and else is returning false
        ...
        if(!isEmpty(process.env.CAV_APP_AGENT_CONNECTINTERVAL)){
            AgentSetting.connectInterval = parseInt(process.env.CAV_APP_AGENT_CONNECTINTERVAL)*1000;
            util.logger.info(AgentSetting.currentTestRun + " | connectInterval is  : "+AgentSetting.connectInterval);
        }
        else{
            util.logger.info(AgentSetting.currentTestRun + " | Default connectInterval is :"+AgentSetting.connectInterval);
        }
        ...
        if(isEmpty(AgentSetting.protocols)){
            if(isEmpty(AgentSetting.ndProjectKey)){
                AgentSetting.protocols = 'TCP';
                util.logger.error(AgentSetting.currentTestRun + " | NDC_COMM_PROTOCOL is not present in environment variable, so assigning it to TCP ");
            }
            else{
                AgentSetting.protocols = 'WSS';
                util.logger.error(AgentSetting.currentTestRun + " | NDC_COMM_PROTOCOL is not present in environment variable, so assigning it to WSS ");
            }
        }
        if(isEmpty(AgentSetting.endpointPrefix)){
            AgentSetting.endpointPrefix = 'Collector';
            util.logger.info(AgentSetting.currentTestRun + " | endpointPrefix is not present in Environment variable , so setting it to Default ");
        }
        if(isEmpty(AgentSetting.appname)){
            AgentSetting.appname = 'Default';
            util.logger.info(AgentSetting.currentTestRun + " | appName is not present in Environment variable , so setting it to Default ");
        }
        if(isEmpty(AgentSetting.tierAttributes)){
            AgentSetting.tierAttributes = 'NA';
            util.logger.info(AgentSetting.currentTestRun + " | tierAttributes is not present in Environment variable , so setting it to Default ");
        }
        if(isEmpty(AgentSetting.ndcHost)){
            AgentSetting.ndcHost = '';
            util.logger.info(AgentSetting.currentTestRun + " | ndcHost is not present in Environment variable , so setting it to Default ");
        }
        if(isEmpty(AgentSetting.ndcPort)) {
            AgentSetting.ndcPort = defaultNdcPort;
            util.logger.info(AgentSetting.currentTestRun + " | ndcPort is not present in Environment variable , so setting it to Default ");
        }
        if(isEmpty(AgentSetting.tier)){
            AgentSetting.tier = 'Default';
            util.logger.info(AgentSetting.currentTestRun + " | Tier is not present in Environment variable , so setting it to Default ");
        }
        if(isEmpty(AgentSetting.server)) {
            AgentSetting.server = os.hostname();
            util.logger.info(AgentSetting.currentTestRun + " | Server is not present in Environment variable , so setting it to "+os.hostname());
        }
        if(isEmpty(AgentSetting.instance)){
            AgentSetting.instance = "";
        }

        NDCServerConfig.resetserverlist();  //resetting server list bcz next to add Server List.
        util.logger.info(AgentSetting.currentTestRun+" | Resetting Server List ");

        if(!isEmpty(AgentSetting.ndProjectKey))            //for SaaS Mode
        {
            ...
            util.logger.info(AgentSetting.currentTestRun + " | Adding SaaS Server ");
        ...
        else{                   //for NoN SaaS Mode
            util.logger.info(AgentSetting.currentTestRun + " | Adding Non SaaS Server ");
...
AgentSetting.readSettingFile = function()
{
    ...
    try {
        if(this.isAgentConfigDefinedInEnv()) {
             util.logger.info(AgentSetting.currentTestRun+" | Checking is agent config in Environment or NOT  ");
        }
...
AgentSetting.getData = function () {
    ...
    try {
        ...
        var connectInterval = properties.get('connectInterval');
        if(!isEmpty(connectInterval)){
            AgentSetting.connectInterval = parseInt(connectInterval)*1000;
            util.logger.info(AgentSetting.currentTestRun + " | connectInterval recived from ndSetting is "+AgentSetting.connectInterval);
        }
        else{
            util.logger.info(AgentSetting.currentTestRun + " | Default connectInterval is "+AgentSetting.connectInterval);
        }
        ...
        if(isEmpty(AgentSetting.protocols)){
            if(AgentSetting.isSaaSMode){
                AgentSetting.protocols = 'WSS';
                util.logger.error(AgentSetting.currentTestRun + " | NDC_COMM_PROTOCOL is not present in ndsetting file , so assigning it to WSS ");
            }
            else{
                AgentSetting.protocols = 'TCP';
                util.logger.error(AgentSetting.currentTestRun + " | NDC_COMM_PROTOCOL is not present in ndsetting file , so assigning it to TCP ");
            }
        }
        if(isEmpty(AgentSetting.endpointPrefix)) {
            AgentSetting.endpointPrefix = 'Collector';
            util.logger.error(AgentSetting.currentTestRun + " | endpointPrefix is not present in ndsetting file , so setting it to default ");
        }
        if(isEmpty(AgentSetting.appname)) {
            AgentSetting.appname = 'Default';
            util.logger.error(AgentSetting.currentTestRun + " | appName is not present in ndsetting file , so setting it to default ");
        }
        if(isEmpty(AgentSetting.tierAttributes)) {
            AgentSetting.tierAttributes = 'NA';
            util.logger.error(AgentSetting.currentTestRun + " | tierAttributes is not present in ndsetting file , so setting it to default ");
        }
        if(isEmpty(AgentSetting.ndcHost)) {
            AgentSetting.ndcHost = '';
