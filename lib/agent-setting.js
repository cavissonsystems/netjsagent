...
AgentSetting.settingFileMode = "exclusive"; // "exclusive" -> Single instance  || "shared" -> Multiple Instances
...
AgentSetting.dumpOnlyMethodExitInFP = 0;
AgentSetting.methodResponseTimeFilter = 0;
AgentSetting.normalResponseTimeFilter = 2;
AgentSetting.slowVerySlowResponseTimeFilter = 5;
...
AgentSetting.getData = function () {
    try {
        ...
        NDCServerConfig.addServers([{host:AgentSetting.ndcHost,port:AgentSetting.ndcPort,type:1},{host:backupNdcHostName,port:backupNdcPort,type:0}])
...
AgentSetting.setMethodResponseTimeFilter = function(value){

  try{
      if(value.toString().indexOf('%20') > -1){
          var resTimeFilter = value.toString().split('%20')
          if(resTimeFilter.length == 3){
              if(resTimeFilter && resTimeFilter[2] > resTimeFilter[1]){

                  AgentSetting.methodResponseTimeFilter = resTimeFilter[0] ? (isNaN(parseInt(resTimeFilter[0])) ? 0 : parseInt(resTimeFilter[0])) : 0
                  AgentSetting.normalResponseTimeFilter = resTimeFilter[1] ? (isNaN(parseInt(resTimeFilter[1])) ? 2 : parseInt(resTimeFilter[1])) : 2
                  AgentSetting.slowVerySlowResponseTimeFilter = resTimeFilter[2] ? (isNaN(parseInt(resTimeFilter[2])) ? 5 : parseInt(resTimeFilter[2])) : 5
              }
          }
      }else{

          AgentSetting.methodResponseTimeFilter = isNaN(parseInt(value)) ? 0 : parseInt(value)
          AgentSetting.normalResponseTimeFilter = 2
          AgentSetting.slowVerySlowResponseTimeFilter = 5
      }
      util.logger.info(AgentSetting.currentTestRun + " | methodResponseTimeFilter value is  : " + AgentSetting.methodResponseTimeFilter,AgentSetting.normalResponseTimeFilter,AgentSetting.slowVerySlowResponseTimeFilter)
  }catch(e){
      util.logger.warn(AgentSetting.currentTestRun + " | Error accured During the Parsing of methodResponseTimeFilter Key")
  }
}
...
var iname = os.networkInterfaces()
AgentSetting.getlocalIP = function(){
    let keys = Object.keys(iname)
    for(let i in keys){
        let values = iname[keys[i]]
        for(let j in values){
            if(values[j].family == 'IPv4' && values[j].internal == false)
                return values[j].address
        }
    }
}
