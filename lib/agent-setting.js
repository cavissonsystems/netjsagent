...
AgentSetting.checkforCorrelationIdInReqRes = function (reqOrResObj, flowpathObj) {
    try {
        if (reqOrResObj) {
            var correlationId = ((reqOrResObj.headers && (reqOrResObj.headers[AgentSetting.correlationIDHeader.toLowerCase()] ||
                reqOrResObj.headers[AgentSetting.correlationIDHeader])) ||
            (reqOrResObj.getHeader && (reqOrResObj.getHeader(AgentSetting.correlationIDHeader.toLowerCase()) ||
    ...
    }catch(err){util.logger.error(AgentSetting.currentTestRun, "| Error occured when getting Co-rrelation id from Request or Response.")}
}
