...
AgentSetting.getData = function (filename) {
    ...
    try {
        ...
        var backupNdcHostAndPort =properties.get('backupNdcHostAndPort');
        var bHost,bPort
        if(backupNdcHostAndPort != null && backupNdcHostAndPort != undefined && backupNdcHostAndPort != 0) {
            var hp = backupNdcHostAndPort.split(':')
            bHost = hp[0]
            bPort = hp[1]
        }
	    NDCServerConfig.addServers([{host:ndcHost,port:ndcPort,type:1},{host:backupNdcHostName,port:backupNdcPort,type:0},
            {host:bHost,port:bPort,type:0}])
        ...
        util.logger.info(AgentSetting.currentTestRun + " | backupNdcHostAndPort : " + backupNdcHostAndPort);
    }catch(err){util.logger.warn(err)}
};
