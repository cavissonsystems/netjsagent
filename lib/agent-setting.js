...
AgentSetting.getData = function () {
    try {
        ...
        if((corelateEventCallback == null) || (corelateEventCallback == undefined) || (corelateEventCallback == 0))
            AgentSetting.corelateEventCallback = 0;
        else {
            if(corelateEventCallback.toString().indexOf('%') > -1){
                var tmp = corelateEventCallback.toString().split('%')
                if(tmp[0] >0) {
                    AgentSetting.corelateEventCallback = tmp[0]
                    AgentSetting.wrapAsyncApi = tmp[1] > 0 ? true : false
                }
            }else
                AgentSetting.corelateEventCallback = corelateEventCallback;
        }
        ...
        util.logger.info(AgentSetting.currentTestRun + " | corelateEventCallback : " + AgentSetting.corelateEventCallback);
    }catch(err){util.logger.warn(err)}
};
