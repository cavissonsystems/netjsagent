...
var autoDiscovery = require('./autoDiscovery');
var handleFPWithForce = require("./flowpath/handleFPWithForce");
var captureProcessCoreDump = require('./captureProcessCoreDump');
var btPercentile = require('./Percentile/ndBTPercentileMon');
var ipPercentile = require('./Percentile/ndIPPercentileMon');

var firstResTimer = undefined;
var isfirstResReceived = false;
var retryCountInvalidToken = 0;
...
//Initial state of TD Monitor
var iStateBT_TD = null,
    iStateIP_TD = null;
...
MessageHandler.prototype.setClientSocket = function(socket,type) {
   try{
       this.clientSocket = socket;

       startInstrProcessing = false;
       agentConfReader.runTimeChange = false;
       instPrfParseobj.resetInstrListOnStart();
       metaData = {data: ''};
       expectedFileMetaData = {size: 0};
       pendingChunk = '';
       clearTimeout(firstResTimer);
       firstResTimer = undefined;
       isfirstResReceived = false;

   }catch(e){
       util.logger.error("Setting Client Socket Error : ",e);
   }
}
...
MessageHandler.prototype.sendIntialMessages = function() {
    ...
    var version = util.netjsagentVersion;
    var build_ver = 0;
    try {
        var last_index_ver = -1;
        last_index_ver = version.lastIndexOf(".");
        if(last_index_ver > -1){
            var last_version_field = version.slice(last_index_ver+1,version.length);
            if(last_version_field.length > 2) {
                build_ver = version.slice(last_index_ver+3);
                version = version.slice(0,parseInt(version.length-build_ver.length));
		        var temp_patch=parseInt(version.slice(-2,-1)-1)
                version=version.slice(0,-2)+temp_patch
            }
        }
    }catch(err){
        util.logger.error("version error  ",err);
    }

    if(agentConfReader.lastConnTimeStamp > 0)
        agentConfReader.connDuration = new Date().getTime() - agentConfReader.lastConnTimeStamp;


    if(agentConfReader.isSaaSMode && agentConfReader.isSaaSServer)
    {
        controlMessage = "cav_saas_auth_agent_msg_req:agentVersion=VERSION " + version + " BUILD " + build_ver + ";agentStartTime="
            + agentConfReader.getBCIStartUpTime() + ";ndHome=" + process.cwd() + ";pid=" + processId + ";agentIp="
            + this.clientSocket.getLocalHost() + ";agentHostName=" + os.hostname() + ";isAgentLess=" + isAgentLess
            + ";jvmType=NodeJS" + ";javaVersion=" + process.version + ";javaHome=" + process.cwd()
            + ";machineType=" + machine + ";supportedProtocols=" + agentConfReader.protocols + ";requestType="
            + agentConfReader.requestType + ";lastConnectionTimeStamp=" + agentConfReader.lastConnTimeStamp + ";connDuration="
            + agentConfReader.connDuration + ";connDestination=" + agentConfReader.lastConnHost + ";projectKey=" + agentConfReader.ndProjectKey
            + ";protoVer=" + agentConfReader.protoversion +";agentType=NodeJS" + ";\n";

        util.logger.info(agentConfReader.currentTestRun+" | Message sent to SaaS : "+controlMessage);
    }
    else
    {
        controlMessage = "nd_ctrl_msg_req:appName=" + agentConfReader.getInstance() + ";ndAppServerHost="
            + agentConfReader.getServerName() + ";tierName=" + agentConfReader.getTierName() + ";bciVersion=VERSION " + version + " BUILD " + build_ver
            + ";bciStartTime=" + agentConfReader.getBCIStartUpTime() + ";ndHome=" + process.cwd() + ";pid=" + processId
            + ";BCITimeStamp=" + new Date().getTime() + ";serverIp=" + this.clientSocket.getLocalHost() + ";hostName=" + os.hostname()
            + ";isAgentLess=" + isAgentLess + ";jvmType=NodeJS" + ";javaVersion=" + process.version
            + ";javaHome=" + process.cwd() + ";machineType=" + machine + ";supportedProtocols=" + agentConfReader.protocols +";agentType=NodeJS;requestType=" + agentConfReader.requestType
            + ";lastConnectionTimeStamp=" + agentConfReader.lastConnTimeStamp + ";connDuration=" + agentConfReader.connDuration
            + ";connDestination=" + agentConfReader.lastConnHost + ":" + agentConfReader.lastConnPort
            + ";tierAttributes=" + agentConfReader.getTierAttributes() + ";applicationName=" + agentConfReader.getappname()
            + ";endpointPrefix="+ agentConfReader.getendpointPrefix() + ";authtoken=" + agentConfReader.authtoken + ";protoVer=" + agentConfReader.protoversion + "\n";

        util.logger.info(agentConfReader.currentTestRun+" | Message sent to ndc : "+controlMessage);
    }
    ...
    */
    ...
    //Timer for FirstResponse from NDC , Time: 1 min.
    if(firstResTimer == undefined && isfirstResReceived == false){
        util.logger.info(agentConfReader.currentTestRun+" | Starting responseTimeoutHandler timer");
        responseTimeoutHandler(this.clientSocket);
    }
...
function stopAllMonitors(){
    try {
        ...
        btPercentile.stopMonitor();
        ipPercentile.stopMonitor();
        if(agentConfReader.dumpFPForcefullyTimer) {
            clearInterval(agentConfReader.dumpFPForcefullyTimer);
            agentConfReader.dumpFPForcefullyTimer=undefined;
            util.logger.info(agentConfReader.currentTestRun + " | Stopping ForceFP Timer on Stop_Instrumentation..");
        }
    }catch(err){util.logger.error("Error in stoping Monitors : ",err)}
}
/*
This method is to handel SaaS Response if Status=OK
 Here we are disintegrating the mater NDC host and master NDC Port and Auth token
*/
function handelSaasAuthResp(clientMsg,clientSocket){
    util.logger.info(agentConfReader.currentTestRun,' | Splitting and Storing data recived from SaaS Controller');
    var saasrep = clientMsg.split(';')
    var masterport
    var masterhost
    var authkey
    for(var k in saasrep){
        var tempstr= saasrep[k].split('=');
        if (saasrep[k].startsWith("host")) {
            masterhost = saasrep[k].substring(5);
        }
        if(saasrep[k].startsWith("port")){
             masterport = saasrep[k].substring(5);
        }
        if(saasrep[k].startsWith("authtoken")){
             authkey =saasrep[k].substring(10);
        }

    }
    agentConfReader.authtoken = authkey;
    agentConfReader.isSaaSServer = false;
    clientSocket.destroy();
    delete clientSocket;
    clientSocket = undefined
    serverConfig.resetserverlist();
    serverConfig.addServers([{host:masterhost,port:masterport,type:1,protocols:agentConfReader.protocols,isSaaSserver:agentConfReader.isSaaSServer}])
    NDConnectionManager.checkProtocolAndMakeConnection();

}
function handleControlConnectionReply(clientMsg,clientSocket){
    isfirstResReceived = true;
    clearTimeout(firstResTimer);
    firstResTimer = undefined;
    if(clientMsg.trim().startsWith('nd_ctrl_msg_rep:result=ERROR;')) {
        ++retryCountInvalidToken;
        if(retryCountInvalidToken>3)
        {
            retryCountInvalidToken=0;
            util.logger.info(agentConfReader.currentTestRun+' | After 3 retry of Master NDC Host & port switching back to SaaS')
    ...
    for(var j in parseData){
        ...
        else if(parseData[j].indexOf('ProtoVersion') > -1){}
        else if(state != 0 && parseData[j].indexOf('supportedProtocols') > -1){
            if(!temp)continue;
            switch_channel = serverConfig.updateActiveProtocol(temp)
        }
    }
    if(state == 0 || switch_channel) {
        util.logger.info(agentConfReader.currentTestRun,'| Closing current Control Channel. DR_State:',state,',Protocl_switing:',switch_channel);
        clientSocket.destroy();
        delete clientSocket;
        clientSocket = undefined ;
        NDConnectionManager.checkProtocolAndMakeConnection();
    ...
    else if(state == 1){            //If state is coming in CC reply & it is enable, then starting timer to send health check
...
function makeDataAutoConnection(){
    try {
        ...
        /* *
        * Getting current protocol ,  if protocol is tcp then we will make object of 'tcp dataConenction' else 'ws'
        * Passing curerent active server , true/false to indicate temporary data connection , current-protocol for port and protol name
        * */

        var currProto =  serverConfig.getCurrentActiveProtocol()

        agentConfReader.dataConnHandler = NDConnectionManager.makeDataConnection(currProto)

        agentConfReader.autoSensorConnHandler = NDConnectionManager.makeAutoConection(currProto)
        ...
        if (agentConfReader.dataConnHandler && agentConfReader.autoSensorConnHandler) {
            ...
            //Starting Force Dump FPs
            if(agentConfReader.FPMaxAllowedAgeInMillis  > 1000) {
                agentConfReader.forceFPDumpInterval = agentConfReader.FPMaxAllowedAgeInMillis * 3;
                util.logger.info(agentConfReader.currentTestRun,"| Interval of forcefully dumping of timed FPs has been started with interval value "+agentConfReader.forceFPDumpInterval);
                handleFPWithForce.dumpFPForcefully(agentConfReader);
            }
            if(agentConfReader.tDigestPercentileBT.enable == 1){
                btPercentile.handleMonitor()
            }
            if(agentConfReader.tDigestPercentileIP.enable == 1){
                ipPercentile.handleMonitor()
            }
        }
...
MessageHandler.prototype.handleMessages = function() {
    ...
    pendingChunk = "";
    clientSocket.handleDataEvent(function (content) {
        var findEOL = false;
        var data= content.toString();

        agentConfReader.lastHeartBeatReceived = new Date().getTime(); // for any data , updating the value.

        util.logger.info(agentConfReader.currentTestRun + " Control message received from ndc/SaaS : " + data + " . FileMetaData : "+ utilcore.inspect(expectedFileMetaData, false)+" , New Line : "+ (data.toString().indexOf('\n') > -1) +" , PendingChunks : "+ (pendingChunk.length ? pendingChunk : 'No Pending Chunk') +'\n');
        ...
        try {
            if(expectedFileMetaData.size > 0) {                 //Parsing file content coming from NDC
                ...
                MessageHandler.readFileContent(fileData,clientSocket,expectedFileMetaData.keywordName);
            }
            else {
            ...
            for(var i=0;i<ndcMsg.length;i++) {
                ...
                if(clientMsg.trim().startsWith("cav_saas_auth_agent_msg_rep:status=OK")){
                    util.logger.info(agentConfReader.currentTestRun,' | Response recived from SaaS Controller with status=OK');
                    util.logger.info(agentConfReader.currentTestRun,' | Sending recived message to split A/C to keyword');
                    handelSaasAuthResp(clientMsg,clientSocket)
                }
                else if(clientMsg.trim().startsWith("cav_saas_auth_agent_msg_rep:status=ERROR")){
                    util.logger.info(agentConfReader.currentTestRun+' | Closing current Control Channel. as Status=ERROR recived from SaaS Controller');
                   clientSocket.closeConn();
                }
                else if(clientMsg.trim().startsWith('nd_ctrl_msg_rep:result=Ok;status=NOT_RUNNING') || clientMsg.trim().startsWith('nd_ctrl_msg_rep:result=Ok;status=RUNNING')){
                    var temp = handleControlConnectionReply(clientMsg,clientSocket);
                    if(temp == true){
                        util.logger.info(agentConfReader.currentTestRun+' |  nd_ctrl_msg_rep:result=Ok;status=NOT_RUNNING is proccessed and returning');
                        return;
                    }
                }
                else if(clientMsg.trim().startsWith('nd_ctrl_msg_rep:result=ERROR;status=NOT_RUNNING') || clientMsg.trim().startsWith('nd_ctrl_msg_rep:result=ERROR;status=RUNNING')){
                    var temp = handleControlConnectionReply(clientMsg,clientSocket);
                    if(temp == true){
                        util.logger.info(agentConfReader.currentTestRun+' | nd_ctrl_msg_rep:result=ERROR;status=NOT_RUNNING is proccessed and returning');
                        return;
                    }
                }
                else if(clientMsg.trim().startsWith("nd_control_req:action=nd_switch_comm_protocol")) {             //parsing health check reply
                    var list=clientMsg.split(';')
                    for(var m in list){
                        if(list[m].indexOf('supportedProtocols') > -1){
                            var temp = list[m].split('=')
                            if(!temp[1])continue;
                            var switch_channel = serverConfig.updateActiveProtocol(temp[1])
                            if(switch_channel){
                                clientSocket.destroy(switch_channel)
                                delete clientSocket;
                                clientSocket = undefined
                                NDConnectionManager.checkProtocolAndMakeConnection()
                            }
                    ...
                    autoSensorConnectionHandler.clearBuffer();
                    ...
                    if (action[1] == "start_instrumentation") {
                        ...
                        try {
                            ...
                            for (var i = 0; i < messageArray.length; i++) {
                                ...
                                else if (propertyValuePairs[0] == "sleepInterval") {
                                    agentConfReader.connectInterval = parseInt(propertyValuePairs[1])*1000;
                                    util.logger.info(currTestId + " | sleepInterval value : " +agentConfReader.connectInterval);
                                }
                                ...
                                * 5- nodejsCpuProfilingTime = This keyword will provide the cpuProfiling waiting time*/
                                ...
                                else if (propertyValuePairs[0] == "enableHSLongStack") {
                                    agentConfReader.setEnableHSLongStackValue(propertyValuePairs[1])
                                }
                                    ...
                                    if(agentConfReader.instance &&  agentConfReader.instance != propertyValuePairs[1]){
                                        agentConfReader.isAppNameSame = true;
                                    }
                                ...
                                else if (propertyValuePairs[0] == "clusterMode") {
                                    if(serverConfig.currentActiveServer.type == 1){
                                        agentConfReader.isClusterMode = propertyValuePairs[1]
                                        util.logger.info(currTestId + " | Cluster Mode (NDCStaticScaling) is : " + agentConfReader.isClusterMode);
                                    }else{
                                        util.logger.error(currTestId + " | Cluster Mode (NDCStaticScaling) Non Primary NDC - Value :",propertyValuePairs[1]);
                                    }
                                }
                                else if (propertyValuePairs[0] == "ndcHeartBeatThreshold") {
                                    var HeartBeatThreshold = parseInt(propertyValuePairs[1])
                                    if(!isNaN(HeartBeatThreshold)){
                                        agentConfReader.ndcHeartBeatThreshold = HeartBeatThreshold;
                                    }
                                    util.logger.info(currTestId + " | ndcHeartBeatThreshold is : " + agentConfReader.ndcHeartBeatThreshold);
                                }
                                    ...
                                    agentConfReader.maxCharInSeqBlob = parseInt(propertyValuePairs[1] / 35);
                                ...
                                else if (propertyValuePairs[0] == "enableCaptureNetDelay") {
                                    agentConfReader.enableCaptureNetDelay = propertyValuePairs[1];
                                    util.logger.info(currTestId + " | enableCaptureNetDelay is  : " + propertyValuePairs[1]);
                                }
                                ...
                                else if (propertyValuePairs[0] == "correlateEventCallback") {
                                    agentConfReader.setCorelateEventCBValue(propertyValuePairs[1]);
                                    util.logger.info(currTestId + " | correlateEventCallback is  : " + propertyValuePairs[1]);
                                    require('./utils/instrumentNodeEvents')(agentConfReader.corelateEventCallback, agentConfReader.wrapAsyncApi)
                                }
                                ...
                                else if(propertyValuePairs[0] == "enableUpdateLogMsgForNF"){
                                    agentConfReader.enableUpdateLogMsgForNF = (parseInt(propertyValuePairs[1]) ? parseInt(propertyValuePairs[1]) : 0) ;
                                    util.logger.info(currTestId + " | enableUpdateLogMsgForNF is (ND-NF Integration) : " + agentConfReader.enableUpdateLogMsgForNF);
                                    if(Object.keys(NDEntryPointManager.entryPointMap).length)
                                        instPrfParseobj.checkAndInstrumentLoggerObject();
                                }
                                else if (propertyValuePairs[0] == "FPMaxAllowedAgeInSec") {
                                    agentConfReader.FPMaxAllowedAgeInMillis = parseInt(propertyValuePairs[1]) ? parseInt(propertyValuePairs[1])*1000 : 0;
                                    util.logger.info(currTestId + " | FPMaxAllowedAgeInMillis's value is  : " + agentConfReader.FPMaxAllowedAgeInMillis);
                                }
                                else if (propertyValuePairs[0] == "enableDumpAsyncId") {
                                    agentConfReader.enableDumpAsyncId = (Number(propertyValuePairs[1]) == 1) ? 1:0;
                                    util.logger.info(currTestId + " | enableDumpAsyncId's value is  : " + agentConfReader.enableDumpAsyncId);
                                }
                                else if (propertyValuePairs[0] == "tDigestPercentileBT") {
                                    parseTDigestPercentileDATA(propertyValuePairs[0],propertyValuePairs[1])
                                    util.logger.info(currTestId + " | tDigestPercentileBT's value is  : " + JSON.stringify(agentConfReader.tDigestPercentileBT));
                                }
                                else if (propertyValuePairs[0] == "tDigestPercentileIP") {
                                    parseTDigestPercentileDATA(propertyValuePairs[0],propertyValuePairs[1])
                                    util.logger.info(currTestId + " | tDigestPercentileIP's value is  : " + JSON.stringify(agentConfReader.tDigestPercentileIP));
                                }
                                else {
                    ...
                    agentConfReader.lastHeartBeatReceived = new Date().getTime();
                    ...
                    if (status == "stopping" || status == "running") {
                        ...
                        if (agentConfReader.dataConnHandler && agentConfReader.autoSensorConnHandler) {
                            ...
                            if(agentConfReader.enableBciDebug > 2){
                                util.logger.info(' Total Transaction :',global.totTran, ',Dumped Transaction :',global.totDumped);
                                global.totTran = 0;
                                global.totDumped = 0;
                            }
                            stopAllMonitors()
                ...
                else if (clientMsg.trim().startsWith("run_async_command_req:")){
                    try{
                        var command = clientMsg.trim().split(';')[0].split('=')[1]
                        var asyncId = clientMsg.split(';')[1].split('=')[1]
                        var currProto = serverConfig.getCurrentActiveProtocol()
                        var dataSocket = NDConnectionManager.makeTemporaryWsDataConnection(currProto)
                        clientSocket.write("run_async_command_rep:Id=" + asyncId + ";Result=Success;\n")
                        var tempClientMsg = clientMsg;
                        dataSocket.createDataConn(serverConfig.currentActiveServer,true,currProto,function(err){
                            dataSocket.client._readableState.highWaterMark = 1024 * 1024 * 15;
                            dataSocket.client._writableState.highWaterMark = 1024 * 1024 * 15;

                            util.logger.info(agentConfReader.currentTestRun+" | Temporary Data Connection established with NDCollector : Socket[addr="+agentConfReader.ndcHost+",port="+agentConfReader.ndcPort + ",localport=" +((currProto.protocol == 'tcp') ? this.localPort : dataSocket.client.socket._socket.address().port)+']');
                            var v8Instance = new v8_profiler();
                            v8Instance.takeHeapSnapShotOnNewConn(clientSocket, dataSocket, tempClientMsg, asyncId, command,function(err){
                                try{
                                    if(err){
                                        var errMsg = "run_async_command_rep:Id="+asyncId+";Result=Error;Output="+err+";\n"
                                        clientSocket.write(errMsg)
                                        util.logger.warn(agentConfReader.currentTestRun + "| Error occured during Taking HeapDump main: "+err)
                                    }
                                    delete v8Instance
                                }
                                catch(e){
                                    util.logger.warn(agentConfReader.currentTestRun + "| Error occured during Taking HeapDump CallBack : "+e)
                                }
                            });
                        })
                    }
                        ...
                        var errMsg = "run_async_command_rep:Id="+asyncId+";Result=Error;Output="+e+";\n"
                        clientSocket.write(errMsg)
                        util.logger.warn(agentConfReader.currentTestRun + " | Unable to take heapDump (Error during Parsing Req.): " + e);
                    }
                }
                else if (clientMsg.trim().startsWith("download_file_req:")){
                    try{
                        var id, fileName, filePath, deleteFile = 0
                        var tempClientMsg = clientMsg;
                        let compressMode, timeOut;
                        var splitMsg = clientMsg.trim().split(";");
                        for (var i = 0; i < splitMsg.length; i++) {
                            if(!splitMsg[i])
                                continue;

                            if (splitMsg[i].indexOf("Id") > -1) {
                                id = splitMsg[i].split(":")[1].split("=")[1];
                            } else if (splitMsg[i].indexOf("TimeOut") > -1) {
                                timeOut = splitMsg[i].split("=")[1];
                            } else if (splitMsg[i].indexOf("FileName") > -1) {
                                fileName = splitMsg[i].split("=")[1];
                            } else if (splitMsg[i].indexOf("FileType") > -1) {
                                filePath = splitMsg[i].split("=")[1];
                            } else if (splitMsg[i].indexOf("DeleteFile") > -1) {
                                deleteFile = splitMsg[i].split("=")[1];
                            }
                        }
                        if (filePath)
                        ...
                        if(!isFileExists)throw new Error('File is not present at path :',fileName)
                        var currProto = serverConfig.getCurrentActiveProtocol()
                        var dataSocket = NDConnectionManager.makeTemporaryWsDataConnection(currProto)

                        clientSocket.write("download_file_rep:Id=" + id + ";Result=Success;\n")
                        ...
                        dataSocket.createDataConn(serverConfig.currentActiveServer,true,currProto,function(err){
                            ...
                            util.logger.info(agentConfReader.currentTestRun+" | Temporary Data Connection established with NDCollector : Socket[addr="+agentConfReader.ndcHost+",port="+agentConfReader.ndcPort + ",localport=" +((currProto.protocol == 'tcp') ? this.localPort : dataSocket.client.socket._socket.address().port) +']' );
                            var v8Instance = new v8_profiler();
                            v8Instance.downloadFileOnNewConn(clientSocket, dataSocket, tempClientMsg, id, fileName, deleteFile,function(err){
                                try{
                                    if(err){
                                        var errMsg = "download_file_rep:Id="+id+";Result=Error;Output="+err+";\n"
                                        clientSocket.write(errMsg)
                                        util.logger.warn(agentConfReader.currentTestRun + "| Error occured during downloading file in controllMsg: "+err)
                                    }
                                    delete v8Instance
                                }
                                catch(e){
                                    util.logger.warn(agentConfReader.currentTestRun + "| Error occured during downloading file CallBack : "+e)
                                }
                            });
                        })
                    }
                        ...
                        var errMsg = "download_file_rep:Id="+id+";Result=Error;Output="+e+";\n"
                        clientSocket.write(errMsg)
                        util.logger.warn(agentConfReader.currentTestRun + " | Unable to download file (Error during Parsing Req.): " + e);
                    }
                }
                   ...
                   // instrProfileParseSuccess = true;//for every instrument profile starting this will be true, if during any failure occur it will false
                    ...
                    //Error case if the file has size=0,
                    instPrfParseobj.isValidExpectedFileMetaData(expectedFileMetaData,clientSocket,makeDataAutoConnection,function(isValid){
                        if(!isValid) {
                            util.logger.error(agentConfReader.currentTestRun,' | Expected Meta Data is not Valid, So resetting the Metadata')
                            metaData={data:''},expectedFileMetaData={size :0}
                        }else
                            util.logger.info(agentConfReader.currentTestRun,' | Expected File Meta Data is Valid.')
                    });
                    if( i < ndcMsg.length -2) {         //Checking if nd_control_rep:action=file msg and data coming in same chunk or not, ig it is same then going to read
                        ...
                        if(data.indexOf('nd_control_rep:action=file') > -1) {
                            ...
                            MessageHandler.readFileContent(msg, clientSocket,expectedFileMetaData.keywordName);
                            break;
                        }
                ...
                else if(clientMsg.trim().startsWith("nd_control_req:action=reroute_ctrl_con_v2;")){

                    MessageHandler.parseReRouteMessageAndUpdateClientInfo(clientMsg);
                    if(!agentConfReader.isClusterMode)
                        MessageHandler.updateNDSettingFile();
                ...
                else if (clientMsg.trim().startsWith("ndc_req:action=discover_loaded_classes;")) {

                    try{
                        var ID, classFilters, methodFilters;
                        var command=clientMsg.split(':');
                        var value = command[1].split(';')
                        for (var a in value){
                            var data = value[a].split('=');
                            if(data[0].indexOf('Id') > -1)
                                ID = parseInt(data[1])
                            if(data[0].indexOf('classFilters') > -1)
                                classFilters = data[1].toString()
                            if(data[0].indexOf('methodFilters') > -1)
                                methodFilters = data[1].toString()
                        }

                        var currProto = serverConfig.getCurrentActiveProtocol()
                        var dataSocket = NDConnectionManager.makeTemporaryWsDataConnection(currProto)
                        clientSocket.write("ndc_rep:action=discover_loaded_classes;Id="+ID+";result=Success;\n");
                        ...
                        dataSocket.createDataConn(serverConfig.currentActiveServer, true, currProto, function (err) {
                            ...
                            try{
                                autoDiscovery.findModules(ID,classFilters,methodFilters,dataSocket)
                            }catch(e){
                                if(dataSocket){
                                    dataSocket.closeConnection()
                                    delete dataSocket
                                }
                                util.logger.warn(agentConfReader.currentTestRun + " | Error in findModules : " + e);
                            }
                        })
                    }catch(e){
                        if(dataSocket){
                        ...
                        util.logger.warn(agentConfReader.currentTestRun + " | Error in discover_loaded_class : " + e);
                    }
                }
                else if(clientMsg.trim().startsWith("nd_control_req:action=captureProcessCoreDump;")) {
                    try {
                        var fileName, commPath;
                        var command = clientMsg.split(':');
                        var value = command[1].split(';')

                        for (var a in value) {
                            var data = value[a].split('=');
                            if (data[0].indexOf('coreDumpCommandPath') > -1)
                                commPath = data[1].toString().trim()
                            if (data[0].indexOf('coreDumpFileName') > -1)
                                fileName = data[1].toString().trim()
                        }
                        captureProcessCoreDump.parseCommand(commPath,fileName,clientSocket)

                    } catch (e) {
                        util.logger.warn(agentConfReader.currentTestRun + 'Error in captureProcessCoreDump', e);
                    }
                }
                else{
...
MessageHandler.readFileContent= function(data,clientSocket,keywordName)  {
    if(!data)return;
    if( pendingChunk != '') {
        //data += pendingChunk +'\n';
        pendingChunk=''
    ...
    if(parseInt(data.length) === parseInt(expectedFileMetaData.size))  {
        isFileBasedOrInstrProfile(data,clientSocket,makeDataAutoConnection,keywordName)
    }
    else if(metaData.data.length === expectedFileMetaData.size){
        isFileBasedOrInstrProfile(metaData.data,clientSocket,makeDataAutoConnection,keywordName)
    }
...
function parseTDigestPercentileDATA(keyword,data){
    try{
        if(keyword && keyword.length > 0 && data && data.length > 0){
            if(data.indexOf('%20') > -1){
                let temp = data.split('%20');
                if(temp.length == 7){
                    var obj ={};
                    obj.enable = Number(temp[0]);
                    obj.aggInterval = temp[1]; //string example (5m , 1h)
                    obj.sMode = Number(temp[2]);
                    obj.delta = Number(temp[3]) > 0 ? Number(temp[3]) : 100;
                    obj.k = Number(temp[4]);
                    obj.groupId = Number(temp[5]);
                    obj.graphId = Number(temp[6]);

                    if(keyword == 'tDigestPercentileBT'){
                        agentConfReader.tDigestPercentileBT = obj;
                        if(agentConfReader.tDigestPercentileBT.enable == 1){
                            btManager.addTD();
                            btPercentile.handleMonitor()
                        }
                        else if(agentConfReader.tDigestPercentileBT.enable == 0){
                            btPercentile.stopMonitor();
                            btManager.removeTD();
                        }
                    }
                    else if(keyword == 'tDigestPercentileIP'){
                        agentConfReader.tDigestPercentileIP = obj;
                        if(agentConfReader.tDigestPercentileIP.enable == 1){
                            backendRecord.addTD();
                            ipPercentile.handleMonitor();
                        }
                        else if(agentConfReader.tDigestPercentileIP.enable == 0){
                            ipPercentile.stopMonitor();
                            backendRecord.removeTD();
                        }
                    }
                    obj = undefined;
                }else{
                    temp = undefined;
                    util.logger.error(agentConfReader.currentTestRun,'|TDigestDATA : Incomplete data')
                }
            }
        }else{
            util.logger.error(agentConfReader.currentTestRun,'| Invalid TDigestPercentile keyword value')
        }
    }catch(e){
        util.logger.error(agentConfReader.currentTestRun,'| Error while parsing the tDigest Keyword Value',e)
    }
}

function isFileBasedOrInstrProfile (data,clientSocket,makeDataAutoConnection,keywordName){
    util.logger.info('Complete data recieved for '+keywordName+' : ',data)
    ...
    if(expectedFileMetaData.type ==="instrumentationProfile")
        ...
        instPrfParseobj.readFileBasedKeywordContent(data, clientSocket,makeDataAutoConnection,keywordName);
...
MessageHandler.parseAllFieldsAndSetInModelIfAllSuccess = function (clientMsg,controlSocket){
    if(clientMsg.indexOf(";") !== -1) {
        ...
        allArguments.length >0 && otherKeywordHandler.parsingKeywordvalue(allArguments)
        for (var i in allArguments) {
            ...
            else if (allArguments[i].startsWith("sleepInterval")) {
                agentConfReader.connectInterval = parseInt(strKeywordValue)*1000;
                util.logger.info(agentConfReader.currentTestRun + " | sleepInterval : "+agentConfReader.connectInterval);
            }
            ...
            else if (allArguments[i].startsWith("enableHSLongStack")) {
                agentConfReader.setEnableHSLongStackValue(strKeywordValue)
                if(agentConfReader.isAsyncEventMonitorEnable)
                    ndAsyncMonitor.filterAndUpdateCommonMap();
                util.logger.info(agentConfReader.currentTestRun + " | Applying run time change for enableHSLongStack is : " + agentConfReader.enableHSLongStack);
            }
            ...
            else if (allArguments[i].startsWith("correlateEventCallback")) {
                agentConfReader.setCorelateEventCBValue(strKeywordValue);
                util.logger.info(agentConfReader.currentTestRun + " | correlateEventCallback is : "+ strKeywordValue);
                require('./utils/instrumentNodeEvents')(agentConfReader.corelateEventCallback, agentConfReader.wrapAsyncApi)
            }
            ...
            else if (allArguments[i].startsWith("enableCaptureNetDelay")) {
                agentConfReader.enableCaptureNetDelay = strKeywordValue
                util.logger.info(agentConfReader.currentTestRun + " | enableCaptureNetDelay : "+agentConfReader.enableCaptureNetDelay);
            }
            ...
            else if (allArguments[i].startsWith("enableUpdateLogMsgForNF=")) {
                if (strKeywordValue > 0) {
                    agentConfReader.enableUpdateLogMsgForNF = strKeywordValue;
                    util.logger.info(agentConfReader.currentTestRun + " | Runtime Change - enableUpdateLogMsgForNF is (ND-NF Integration) : " + agentConfReader.enableUpdateLogMsgForNF);
                    instPrfParseobj.checkAndInstrumentLoggerObject();
                }else{
                    agentConfReader.enableUpdateLogMsgForNF = 0;
                    agentConfReader.mapForWinstonLogMeths = {};
                    agentConfReader.mapForConsoleMeths = {};
                    util.logger.info(agentConfReader.currentTestRun + " | Runtime Change - enableUpdateLogMsgForNF is (ND-NF Integration) : " + agentConfReader.enableUpdateLogMsgForNF);
                }
            }
            else if(allArguments[i].startsWith("FPMaxAllowedAgeInSec")){
                if(strKeywordValue > 1) {
                    agentConfReader.FPMaxAllowedAgeInMillis = parseInt(strKeywordValue) ? parseInt(strKeywordValue) * 1000 : 0;
                    util.logger.info(agentConfReader.currentTestRun + " | Applying run time - Starting timer for forcefully dumping flowpath , FPMaxAllowedAgeInMillis keyword value is : "+ agentConfReader.FPMaxAllowedAgeInMillis)
                    if (agentConfReader.FPMaxAllowedAgeInMillis  > 1000) {
                        agentConfReader.forceFPDumpInterval = agentConfReader.FPMaxAllowedAgeInMillis * 3;
                        handleFPWithForce.dumpFPForcefully(agentConfReader);
                    }
                    ...
                    if(agentConfReader.dumpFPForcefullyTimer) {
                        clearInterval(agentConfReader.dumpFPForcefullyTimer);
                        agentConfReader.dumpFPForcefullyTimer=undefined;
                        util.logger.info(agentConfReader.currentTestRun + " | Applying run time - Stoping timer for forcefully dumping flowpath ");
                    }
                }
            }
            else if (allArguments[i].startsWith("enableDumpAsyncId")) {
                agentConfReader.enableDumpAsyncId = (Number(strKeywordValue) == 1)?1:0;
                util.logger.info(agentConfReader.currentTestRun + " | enableDumpAsyncId : "+agentConfReader.enableDumpAsyncId);
            }
            else if (allArguments[i].startsWith("tDigestPercentileBT")) {
                parseTDigestPercentileDATA(strKeyword,strKeywordValue);
                util.logger.info(agentConfReader.currentTestRun + " | tDigestPercentileBT : " + JSON.stringify(agentConfReader.tDigestPercentileBT));
            }
            else if (allArguments[i].startsWith("tDigestPercentileIP")) {
                parseTDigestPercentileDATA(strKeyword,strKeywordValue);
                util.logger.info(agentConfReader.currentTestRun + " | tDigestPercentileIP : " + JSON.stringify(agentConfReader.tDigestPercentileIP));
            }
        }
    }
...
MessageHandler.handleRunTimeChangeCase = function(clientMsg,controlSocket) {
	...
	if ((clientMsg.indexOf("HTTPStatsCondCfg") != -1) || (clientMsg.indexOf("NDHTTPRe") != -1)
        ...
        ndExceptionCaptureSettings.parseExceptionCaptureSettings(clientMsg);
        util.logger.info(agentConfReader.currentTestRun + " | Client Message sent to parseExceptionCaptureSettings to check Excceptions");
        ...
        if (clientMsg.indexOf("NDEntryPointsFile=NA") != -1){
            NDEntryPointManager.reset();
            //controlSocket.write("nd_control_rep:action=modify;result=Ok;" + '\n');
            //return;
        }
...
MessageHandler.parseReRouteMessageAndUpdateClientInfo = function(clientmsg){
    ...
    try {
        var allFields = clientmsg.split(";");
        var ndcHost , ndcPort , bkpNdcHost , bkpNdcPort ,protocolType ;
        for (var eachKeyVal in allFields) {
            ...
            if(key == "clusterMode"){
                agentConfReader.isClusterMode = parseInt(all[1])
                if(agentConfReader.isClusterMode)
                    agentConfReader.requestType = 2;
            }else if(key == 'protocolType'){
                protocolType = all[1]
            }else if (key == "ndcHost") {
                ndcHost = all[1];
                if (ndcHost)
        ...
        var val = agentConfReader.protocols;
        if(val !== undefined || val != null || val != 0 || val.length > 0){
            protocolType = protocolType+','+val;
        }
        if(!agentConfReader.isClusterMode){
            serverConfig.addServers([{host:agentConfReader.ndcHost,port:agentConfReader.ndcPort,type:1,protocols : protocolType}])
            serverConfig.currentActiveServer = serverConfig.serverList[0];
        }else{
            if(serverConfig.serverList.length > 1){
                util.logger.info(agentConfReader.currentTestRun+' | Removing first index from serverlist ',serverConfig.serverList)
                serverConfig.serverList.pop();
            }
            serverConfig.addServers([{host:agentConfReader.ndcHost,port:agentConfReader.ndcPort,type:0,protocols : protocolType}])
            serverConfig.currentActiveServer = serverConfig.serverList[1];
            util.logger.info(agentConfReader.currentTestRun+' | Setting the Assinged NDC ip:port on Index 1.',serverConfig.serverList)
        }
        if ( bkpNdcHost && bkpNdcPort && !agentConfReader.isClusterMode) {
            serverConfig.addServers([{host:agentConfReader.ndcHost,port:agentConfReader.ndcPort,type:0,protocols : protocolType}])
            util.logger.info(agentConfReader.currentTestRun +'Setting Backup NDC ip:port at index 1')
        }
        /*if(agentConfReader.isClusterMode){
            if(agentConfReader.reconnectTimer != undefined){
                clearInterval(agentConfReader.reconnectTimer)          //Clearing reconnect timer interval
                agentConfReader.reconnectTimer = undefined;
            }
        }*/
...
function responseTimeoutHandler(clientSocket){
    //Used for setting 1 minute time Period to break  the CC ,if First Response in not received.
    try{
        firstResTimer = setTimeout(function(){
            if(!isfirstResReceived) {
                if(clientSocket && !clientSocket.isReconnectInProgress){
                    util.logger.warn(agentConfReader.currentTestRun + " | Unable to Receive the First nd_ctrl_msg Hence, Breaking the connection.: " );
                    clientSocket.destroy();
                    delete clientSocket;
                    clientSocket = undefined
                    NDConnectionManager.checkProtocolAndMakeConnection();
                }else{
                    util.logger.warn(agentConfReader.currentTestRun + " | Control-Connection reconnect Process is in Progress ." );
                }
            }
            else if(isfirstResReceived){
                isfirstResReceived = false;
            }
            clearTimeout(firstResTimer);
            firstResTimer = undefined;
        ...
        util.logger.warn(agentConfReader.currentTestRun + " | Exception in First Response Timer :" + e);
    }
}
