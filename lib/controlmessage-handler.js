...
*/
    ...
    runTimeChange = false ;startInstrProcessing = false
}
    ...
    clientSocket.on("data", function (content) {
        ...
        try {
            if(expectedFileMetaData.size > 0) {                 //Parsing file content coming from NDC
                if(data.trim().startsWith("nd_control_rep:action=reconnect;result=Ok")) {
                    healthCheckReply = true;
                    var first = data.indexOf("\n");
                        data = data.substring(first+1,data.length);
                }
                MessageHandler.readFileContent(data.toString(),clientSocket);
            }
                ...
                if(clientMsg.trim().startsWith('nd_ctrl_msg_rep:result=Ok;status=NOT_RUNNING') || clientMsg.trim().startsWith('nd_ctrl_msg_rep:result=Ok;status=RUNNING')){
                    ...
                    for(var j in parseData){
                        ...
                        if(parseData[j].indexOf('state') > -1) {
                            serverConfig.currentActiveServer.state = temp
                            if(serverConfig.currentActiveServer.state == 1){            //If state is coming in CC reply & it is enable, then starting timer to send health check
                                //serverConfig.currentActiveServer.type = Active
                            ...
                            serverConfig.sleepInterval = parseInt(policy[1] * 600)
                        }
                    ...
                    if (action[1] == "start_instrumentation") {
                        try {
                            ...
                            if (agentConfReader.previousTestRun !== currTestId) {
                                ...
                                NDSessionCaptureSettings.resetValues();         //Clearing all values for session attr capturing and custom header data capturing
...
MessageHandler.startHealthCheckTimer = function(clientSocket) {
    if(agentConfReader.reconnectTimer == undefined) {           //If timer is defined then no need to run new timer
        agentConfReader.reconnectTimer = setInterval(function () {
            if(runTimeChange || startInstrProcessing)           // If agent is busy with NDC, then dont send health check msg
                return;

            if (healthCheckReply == false)++healthCheckReplyCount;

            healthCheckReply = false;
            if (healthCheckReplyCount >= healthCheckReplyLimit) {           //After failing for some particular time, agent will switch over and close connection
                healthCheckReplyCount=-1;
                serverConfig.currentActiveServer.type = Backup
            ...
            util.logger.warn(agentConfReader.currentTestRun+" | HeartBeat :",msg)
