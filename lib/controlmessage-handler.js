            ...
            for(var i=0;i<ndcMsg.length-1;i++) {
                ...
                if(clientMsg.trim().startsWith('nd_ctrl_msg_rep:result=Ok;status=NOT_RUNNING') || clientMsg.trim().startsWith('nd_ctrl_msg_rep:result=Ok;status=RUNNING')){
                        ...
                        try {
                            ...
                            for (var i = 0; i < messageArray.length; i++) {
                                ...
                                * 5- nodejsCpuProfilingTime = This keyword will provide the cpuProfiling waiting time*/
                                    ...
                                    agentConfReader.enableHSLongStack = propertyValuePairs[1]
                                    util.logger.info(currTestId + " | enableHSLongStack : "+agentConfReader.enableHSLongStack);
                                }
                    ...
                    try{
                        ...
                        dataSocket = new dataConnectionHandler();
                        dataSocket.client.on('connect',function(err){
                            ...
                            v8_profiler.takeHeapSnapShotOnNewConn(clientSocket, dataSocket, clientMsg, asyncId,command);
                        })
    ...
    if(clientMsg.indexOf(";") !== -1) {
        ...
        for (var i in allArguments) {
            ...
            if (argumentAndValue == null || argumentAndValue.length != 2) {
                ...
                agentConfReader.enableHSLongStack = strKeywordValue;
