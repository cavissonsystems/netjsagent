...
function MessageHandler(clientSocket,type)
{
    this.clientSocket = clientSocket;
    this.type = type;
    /*If type is net.socket then we will register data event single time
     * */
    if(this.type == 'net') this.handleMessages();
}

MessageHandler.prototype.setClientSocket = function(socket,type) {
    this.clientSocket = socket;
    this.type = type
    /*If type is ws socket then we will register data event multiple time
     This has been shifted from constructor to 'setClientSocket' because every time we have to register for data event over
     new connection instance , in constructor it was setting only single time
     * */
    if(this.type == 'ws') this.handleMessages();
}
...
function makeDataAutoConnection(){
    try {
        ...
        agentConfReader.dataConnHandler = (agentConfReader.webSocketMode == 0 ? new dataConnectionHandler() :
            new ws_dataConnectionHandler());
        agentConfReader.dataConnHandler.createDataConn(serverConfig.currentActiveServer);

        agentConfReader.autoSensorConnHandler = (agentConfReader.webSocketMode == 0 ? new AutoSensorConnectionHandler() :
            new ws_AutoSensorConnectionHandler());
...
MessageHandler.prototype.handleMessages = function() {
    ...
    pendingChunk = "";
    var client_conn = clientSocket.getControlSocket();
    var eventName = (this.type == 'ws'?'message':'data')
    client_conn.on(eventName, function (content) {
        var findEOL = false;
