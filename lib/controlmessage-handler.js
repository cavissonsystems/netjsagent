...
MessageHandler.prototype.setClientSocket = function(socket) {
   ...
   try{
       ...
       if(!this.clientSocket.listeners('data').length){
           util.logger.error("Data Event Not Found So Attaching data Event. ");
           this.handleMessages();
       }
   }catch(e){
...
MessageHandler.prototype.handleMessages = function() {
    ...
    clientSocket.on("data", function (content) {
        ...
        //To get the Received Message Info and State of Agent.
        
        //util.logger.info(agentConfReader.currentTestRun + " | Control message received from ndc : " + data);
        //util.logger.info(agentConfReader.currentTestRun + " | ExpectedFileMetaData Info : " + utilcore.inspect(expectedFileMetaData, false));
        //util.logger.info(agentConfReader.currentTestRun + " | Message contains New Line for Incoming Message received : " + (data.toString().indexOf('\n') > -1));
        //util.logger.info(agentConfReader.currentTestRun + " | Pending Chunk : " + (pendingChunk.length ? pendingChunk : 'No Pending Chunk') +'\n');
        util.logger.info(agentConfReader.currentTestRun + " | Message : " + data + " FileMetaData : "+ utilcore.inspect(expectedFileMetaData, false)+" New Line : "+ (data.toString().indexOf('\n') > -1) +" Chunks : "+ (pendingChunk.length ? pendingChunk : 'No Pending Chunk') +'\n');
            ...
            for(var i=0;i<ndcMsg.length;i++) {
                ...
                if(clientMsg.trim().startsWith('nd_ctrl_msg_rep:result=Ok;status=NOT_RUNNING') || clientMsg.trim().startsWith('nd_ctrl_msg_rep:result=Ok;status=RUNNING')){
                    ...
                    try{
                        ...
                        dataSocket = new dataConnectionHandler(true);
                        dataSocket.client.on('connect',function(err){
                            util.logger.info(agentConfReader.currentTestRun+" | Temporary Data Connection established with NDCollector : Socket[addr="+agentConfReader.ndcHost+",port="+agentConfReader.ndcPort + ",localport=" +this.localPort );
                            var v8Instance = new v8_profiler();
                            v8Instance.takeHeapSnapShotOnNewConn(clientSocket, dataSocket, clientMsg, asyncId, command,function(err){
                                try{
                                    if(err){
                        ...
                        dataSocket.client.on('error',function(err){
                            util.logger.warn(agentConfReader.currentTestRun + "| Error occured during connecting to client "+err)
                        })
                        clientSocket.write("run_async_command_rep:Id=" + asyncId + ";Result=Success;\n")
                        dataSocket.createDataConn(serverConfig.currentActiveServer,true)
                    }
                    catch (e){
                        if(dataSocket){
                    ...
                    try {
                        ...
                        dataSocket.client.on('connect',function(err){
                            util.logger.info(agentConfReader.currentTestRun+" | Temporary Data Connection established with NDCollector : Socket[addr="+agentConfReader.ndcHost+",port="+agentConfReader.ndcPort + ",localport=" +this.localPort );
                            var v8Instance = new v8_profiler();
                            v8Instance.downloadFileOnNewConn(clientSocket, dataSocket, clientMsg, id, fileName, deleteFile,function(err){
                                try{
                                    if(err){
                                        var errMsg = "download_file_rep:Id="+id+";Result=Error;Output="+err+";\n"
                                        clientSocket.write(errMsg)
                                        util.logger.warn(agentConfReader.currentTestRun + "| Error occured during downloading file main: "+err)
                                    }
                                    delete v8Instance
                                }
                                catch(e){
                        ...
                        clientSocket.write(errMsg)
                        util.logger.warn(agentConfReader.currentTestRun + " | Unable to  downloading file (Error during Parsing Req.): " + e);
                    }
                }
