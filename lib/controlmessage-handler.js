                        ...
                        try {
                            ...
                            for (var i = 0; i < messageArray.length; i++) {
                                ...
                                * 5- nodejsCpuProfilingTime = This keyword will provide the cpuProfiling waiting time*/
                                ...
                                }else if ( propertyValuePairs[0] == "dumpOnlyMethodExitInFP"){
                                    agentConfReader.dumpOnlyMethodExitInFP = propertyValuePairs[1]
                                    util.logger.info(currTestId + " | DumpOnlyMethodExitInFP is  : " + agentConfReader.dumpOnlyMethodExitInFP);
                                }
                                else if ( propertyValuePairs[0] == "methodResponseTimeFilter"){
                                    agentConfReader.setMethodResponseTimeFilter(propertyValuePairs[1])
                                }
                                else {
...
MessageHandler.parseAllFieldsAndSetInModelIfAllSuccess = function (clientMsg,controlSocket){
    if(clientMsg.indexOf(";") !== -1) {
        ...
        for (var i in allArguments) {
            ...
            else if (allArguments[i].startsWith("dumpOnlyMethodExitInFP")){
                agentConfReader.dumpOnlyMethodExitInFP = propertyValuePairs[1]
                util.logger.info(currTestId + " | DumpOnlyMethodExitInFP is  : " + agentConfReader.dumpOnlyMethodExitInFP);
            }
            else if (allArguments[i].startsWith("methodResponseTimeFilter")) {
                agentConfReader.setMethodResponseTimeFilter(strKeywordValue)
            }
