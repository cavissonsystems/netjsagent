...
MessageHandler.parseReRouteMessageAndUpdateClientInfo = function(clientmsg){
    ...
    try {
        ...
        if(!agentConfReader.isClusterMode){
            ...
            serverConfig.serverList[0].protocolList[0].port = ndcPort
            ...
            util.logger.info('Seeting the Assinged NDC ip:port on Index 1.',serverConfig.serverList)
        }
        if ( bkpNdcHost && bkpNdcPort && !agentConfReader.isClusterMode) {
            ...
            util.logger.info('Setting Backup NDC ip:port at index 1')
        }
        /*if(agentConfReader.isClusterMode){
