    ...
    clientSocket.on("data", function (content) {
        ...
        try {
            ...
            for(var i=0;i<ndcMsg.length-1;i++) {
                ...
                if (clientMsg.trim().startsWith("nd_control_req:action=bci_stat;")) {
...
MessageHandler.readFileContent= function(data,clientSocket)  {
    ...
    if(data.length===expectedFileMetaData.size)  {
        metaData={data:''},expectedFileMetaData={size :0}
        metaData.size=0;
        /*if(instPrfParseobj.expectingContentFileType==="InstrumentationProfile") {
         instPrfParseobj.readInstrumentationProfile(data, clientSocket,makeDataAutoConnection);
         }
         else*/ {
    ...
    else if(data.length < expectedFileMetaData.size) {
        //metaData.remainingSize=metaData.size-data.length;
        metaData.data +=data ;
        if(metaData.data.length === expectedFileMetaData.size){
            instPrfParseobj.readFileBasedKeywordContent(metaData.data, clientSocket,makeDataAutoConnection);
            metaData={data:''},expectedFileMetaData={size :0}
            if(runTimeChange) {
                clientSocket.write("nd_control_rep:action=modify;result=Ok;" + '\n');
        ...
        //handle case of data.length >= metaData.size
    }
}
