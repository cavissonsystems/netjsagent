    ...
    clientSocket.on("data", function (content) {
        ...
        try {
            ...
            for(var i=0;i<ndcMsg.length-1;i++) {
                ...
                else if (clientMsg.trim().startsWith("run_async_command_req:Command=nd_meta_data_req:action=get_heap_dump;")){
                    try{
			var msg = clientMsg.split(';')
			...
			if(msgLength == 9){
                            ...
                            MessageHandler.handleClientMessageForTakingHeapDumpOnNewConn(clientSocket, dataSocket, clientMsg);
                        }
		        else{
                            var errMsg = "run_async_command_rep:Id="+asyncId+";Result=Error;Output=Full Msg Not Recieved."+"\n"
                            clientSocket.write(errMsg)
			    util.logger.warn(agentConfReader.currentTestRun + " | Error in recieving the full msg : ");
...
MessageHandler.handleClientMessageForTakingHeapDumpOnNewConn = function(clientSocket,dataSocket,clientMsg){
	
        v8_profiler.takeHeapSnapShotOnNewConn(clientSocket,dataSocket,clientMsg);
}
