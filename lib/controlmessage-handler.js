                        ...
                        try {
                            ...
                            for (var i = 0; i < messageArray.length; i++) {
                                ...
                                * 5- nodejsCpuProfilingTime = This keyword will provide the cpuProfiling waiting time*/
                                    ...
                                    agentConfReader.enableUpdateLogMsgForNF = parseInt(propertyValuePairs[1]) ;
        ...
        for (var i in allArguments) {
            ...
            if (argumentAndValue == null || argumentAndValue.length != 2) {
                ...
                if (strKeywordValue > 0) {
                    ...
                    util.logger.info(agentConfReader.currentTestRun + " | enableUpdateLogMsgForNF is (ND-NF Integration) : " + agentConfReader.enableUpdateLogMsgForNF);
                    var instrumentConsole = require('./nodetime/lib/probes/console');
                       // instrumentWinston = require('./nodetime/lib/probes/winston');
                    if (NDEntryPointManager.entryPointMap['CONSOLE'] && NDEntryPointManager.entryPointMap['CONSOLE'].enable == 1) {
                        instrumentConsole.instrumentLoggerObj(console, NDEntryPointManager.entryPointMap['CONSOLE'].method);
                    }
                    if (NDEntryPointManager.entryPointMap['WINSTON'] && NDEntryPointManager.entryPointMap['WINSTON'].enable == 1 && NDEntryPointManager.entryPointMap['WINSTON'].method) {
                        agentConfReader.mapForWinstonLogMeths={};
                        //instrumentWinston.instrumentLoggerObj(util.logger, NDEntryPointManager.entryPointMap['WINSTON'].method);
                        var methArr = NDEntryPointManager.entryPointMap['WINSTON'].method.split(",");
                        for(var i in methArr)
                            agentConfReader.mapForWinstonLogMeths[methArr[i]]=true;
                    }
                }
            }
        }
