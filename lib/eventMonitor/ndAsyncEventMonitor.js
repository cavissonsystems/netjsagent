...
function pre(uid) {
    ...
    if (AgentSetting.enableHSLongStack == 1 && delay > asSettingObj.threshold) {
        ...
        asManagerFile.handledHotspotData(getStack("", uid), delay, eventIdNameMap[uid].startTime - AgentSetting.cavEpochDiffInMills, eventIdNameMap[uid].fpId, "LongStack", process.pid, (date - AgentSetting.cavEpochDiffInMills));
    }
}
function post(uid, didThrow) {
    ...
    currentUid = -1;
    if (AgentSetting.enableHSLongStack == 1 && executionTime > asSettingObj.threshold) {
        asManagerFile.handledHotspotData(getStack("", uid), executionTime, eventIdNameMap[uid].startTime - AgentSetting.cavEpochDiffInMills, eventIdNameMap[uid].fpId, "LongStack", process.pid, (new Date().getTime() - AgentSetting.cavEpochDiffInMills));
    }
...
function before(uid){
    if(eventIdNameMap[uid]){
        ...
        asyncDataModel.updateAsyncEventDelay(eventIdNameMap[uid].provider,delay);
        currentUid = uid;
        if (AgentSetting.enableHSLongStack == 1 && delay > asSettingObj.threshold && asyncDataModel.asyncEventDataMap[eventIdNameMap[uid].provider]) {
            var requestedObj=AgentSetting.getFlowPathIdFromRequest();
            if(requestedObj) {
                if (requestedObj.cavFlowPathId)
                    eventIdNameMap[uid].fpId = requestedObj.cavFlowPathId;
...
function after(uid) {
    if(eventIdNameMap[uid]){
        ...
        asyncDataModel.updateAsyncEventExecTime(eventIdNameMap[uid].provider,executionTime);
        currentUid = -1;
        if (AgentSetting.enableHSLongStack == 1 && executionTime >  asSettingObj.threshold && asyncDataModel.asyncEventDataMap[eventIdNameMap[uid].provider]) {
            asManagerFile.handledHotspotData(getStack("", uid), executionTime, eventIdNameMap[uid].startTime - AgentSetting.cavEpochDiffInMills, eventIdNameMap[uid].fpId, "LongStack", process.pid, (new Date().getTime() - AgentSetting.cavEpochDiffInMills));
        }
    }else{
