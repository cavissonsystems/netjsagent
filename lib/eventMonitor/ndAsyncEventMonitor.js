...
asyncEventMon.start = function() {
    try {
        ...
        asyncEventMon.filterAndUpdateCommonMap();
        if(enableAsyncHooks){
    ...
    try {
        ...
        if (event) {
            ...
            if (AgentSetting.enableHSLongStack > 0) {
                ...
                if (delay > asSettingObj.threshold && event.fpId && event.fpId != -1) {
...
function post(uid, didThrow) {
    ...
    try {
        ...
        if (event) {
            ...
            currentUid = -1;
            if (AgentSetting.enableHSLongStack > 0 && executionTime > asSettingObj.threshold && event.fpId && event.fpId != -1) {
...
asyncEventMon.create87Record = function(id, value){
    ...
    asyncEventData += ' ';
    asyncEventData += ((value.totExecutionTime > 0) && Number(value.eventCount) > 0) ? (Number(value.totExecutionTime)/Number(value.eventCount)).toFixed(3) : value.totExecutionTime;
    asyncEventData += ' ';
    asyncEventData += (value.eventCount > 0) ? ((value.minTime == Number.MAX_VALUE)?0:value.minTime):0;
    asyncEventData += ' ';
    asyncEventData += value.maxTime;
    asyncEventData += ' ';
    asyncEventData += value.eventCount;
    asyncEventData += ' ';
    asyncEventData += ((value.timeDelay > 0) && Number(value.eventCount) > 0 )? (Number(value.timeDelay)/Number(value.eventCount)).toFixed(3) : value.timeDelay;
    asyncEventData += ' ';
    asyncEventData += (value.eventCount > 0) ? ((value.minDelay == Number.MAX_VALUE)?0:value.minDelay):0;
    asyncEventData += ' ';
...
asyncEventMon.filterAndUpdateCommonMap = function(){
    util.logger.info(AgentSetting.currentTestRun, '| Updating Asynchronous resources .');
    ...
    asyncDataModel.asyncEventDataMap={};
    if (enableAsyncHooks) {
