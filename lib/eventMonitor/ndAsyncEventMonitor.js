...
function init(uid, provider, parentUid, parentHandle) {
    ...
    if(AgentSetting.enableHSLongStack == 2 ) {
        ...
        // Compute the full stack and store on the `Map` using the `uid` as key.
        var longStack = localStack.concat(extraStack);
        stack[uid] = (extraStack) ? (longStack.length > 500 ? longStack.splice(0,longStack.length - 300):longStack): localStack;//eventIdNameMap[uid].stackArr;//localStack + '\n' + extraStack;
    }
...
function pre(uid) {
    ...
    currentUid = uid;
    if (AgentSetting.enableHSLongStack == 1 && delay > asSettingObj.threshold) {
        ...
        if(eventIdNameMap[uid].fpId != -1) {
            if (AgentSetting.enableHSLongStack == 1)
                stack[uid] = (new Error()).stack.split("\n");
            else
                stack[uid] = getStack("", uid);

            asManagerFile.handledHotspotData(stack[uid], delay, eventIdNameMap[uid].startTime - AgentSetting.cavEpochDiffInMills, eventIdNameMap[uid].fpId, "LongStack", process.pid, (date - AgentSetting.cavEpochDiffInMills));
        }
    }
}
function post(uid, didThrow) {
    ...
    currentUid = -1;
    if (AgentSetting.enableHSLongStack > 0 && executionTime > asSettingObj.threshold) {
        if(eventIdNameMap[uid].fpId != -1) {
            if (AgentSetting.enableHSLongStack == 1)
                stack[uid] = (new Error()).stack.split("\n");
            else
                stack[uid] = getStack("", uid);
            asManagerFile.handledHotspotData(stack[uid], executionTime, eventIdNameMap[uid].startTime - AgentSetting.cavEpochDiffInMills, eventIdNameMap[uid].fpId, "LongStack", process.pid, (new Date().getTime() - AgentSetting.cavEpochDiffInMills));
        }
    }
...
function before(uid){
    if(eventIdNameMap[uid]){
        ...
        asyncDataModel.updateAsyncEventDelay(eventIdNameMap[uid].provider,delay);
        currentUid = uid;
        if (AgentSetting.enableHSLongStack > 0 && delay > asSettingObj.threshold && asyncDataModel.asyncEventDataMap[eventIdNameMap[uid].provider]) {
            var requestedObj=AgentSetting.getFlowPathIdFromRequest();
            if(requestedObj) {
                if (requestedObj.cavFlowPathId)
                    eventIdNameMap[uid].fpId = requestedObj.cavFlowPathId;
...
function after(uid) {
    if(eventIdNameMap[uid]){
        ...
        asyncDataModel.updateAsyncEventExecTime(eventIdNameMap[uid].provider,executionTime);
        currentUid = -1;
        if (AgentSetting.enableHSLongStack > 0 && executionTime > asSettingObj.threshold && asyncDataModel.asyncEventDataMap[eventIdNameMap[uid].provider]) {
            if(eventIdNameMap[uid].fpId != -1) {
                if(AgentSetting.enableHSLongStack ==1)
                    stack[uid] = (new Error()).stack.split("\n");
                else
