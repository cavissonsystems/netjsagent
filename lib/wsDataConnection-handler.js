...
var connectConnListener = function() {
    try {
        ...
        if(!self.istempDataConn){
            util.logger.info(agent.currentTestRun+" | Data Connection established with NDCollector : Socket[addr="+self.ndcHost+",port="+self.ndcPort + ",localport=" +self.getLocalPort()+"]" );
...
dataConnHandler.prototype.getLocalPort = function(){
    try {
...
dataConnHandler.prototype.createDataConn = function(server,istempDataConn,currProto){
    try {
...
dataConnHandler.prototype.closeConnection =function() {
    try {
        ...
        if (this.client) {
            ...
            this.client.removeListener('open',connectConnListener);
            this.client.close();
...
dataConnHandler.prototype._connect = function() {
    ...
    try {
        ...
        this.client.on('open',connectConnListener);
            ...
            util.logger.warn(agent.currentTestRun+" | Error in making data connection");
        }
};
    ...
    try {
        ...
        if(this.client.readyState === WebSocket.OPEN) {
            if(!this.istempDataConn) {
                if (this.client.bufferedAmount >= agent.ndDataBufferSize) {
