...
var fs = require('fs')
var fnmatch = require('./fnmatch.js');
...
instrumentationProfleParser.parseRunTimeInstrumentationProfile = function(list){
    ...
    //cleaning the runTimeInstrumentation Map
    runTimeInstrProfMap = {}
    for (var i in jsn) {
        runTimeInstrProfMap[jsn[i].modulename] = jsn[i];
    }
    console.log(runTimeInstrProfMap)
    transformModules();
}
...
function transformModules() {
    try {
        ...
        for (var module in runTimeInstrProfMap) {
            ...
            //var abc= fnmatch(moduleName,self.config.files)
             if (module.charAt(module.length - 1) == '*') {
                 ...
                 var readFile = function (moduleName, list, callback) {
                     ...
                     for (var i in files) {
                         if (fs.statSync(moduleName + '/' + files[i]).isDirectory())
                             ...
                             let relPath = path.relative(process.cwd(), moduleName + files[i]);
                             //fnmatch return true if present
                             let instrument = fnmatch(relPath, NDEntryPointManager.files);
                             if(!instrument)list.push([moduleName + files[i], runTimeInstrProfMap[module].instrument])
                         }
                     }
                     return (list);
                 }
                 ...
                 let relPath = path.relative(process.cwd(), moduleName);
                 //fnmatch return true if present
                 let instrument = fnmatch(relPath, NDEntryPointManager.files);
                 if(!instrument)valiateAndInstrument([[moduleName, runTimeInstrProfMap[module].instrument]])
             }
        }
    }
...
function valiateAndInstrument(moduleName){
   try{
       ...
       util.logger.error("\n Error in trace valiateAndInstrument ",e)
   }
}
