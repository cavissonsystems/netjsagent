...
function Tracer(formatters) {
	...
	this.onEntryArgs = { name: '', file: '', line: 0, args: null, stack: null, cavisson_req: null };
	this.onExitArgs = { name: '', file: '', line: 0, retLine: 0, span: 0, stack: null, exception: false, returnValue: null, arguments: null, cavisson_req: null };
}
...
Tracer.prototype.onEntry = function(args) {
	...
	var sid = this.pushFrame(stackFrame, args.args);
	...
	this.onEntryArgs.cavisson_req = args.cavisson_req;

	//this.onEntryArgs.cavissonId = this
	...
	//Changed by Alok- returning original arguments used to invoked method also from here date- 04/03/2016
	return {name: args.name, file: args.file, fnLine: args.line, ts: Date.now(), stackId: sid, cavisson_req: args.cavisson_req,args: args.args};
};
...
Tracer.prototype.onExit = function(args) {
	...
	this.onExitArgs.cavisson_req = args.entryData.cavisson_req;
	var frame = this.popFrame(args.entryData.stackId);



	try {
		//this.onExitArgs.arguments = args.args;
		//Changed by Alok- setting  original arguments from entry data passed on method entry date- 04/03/2016
		this.onExitArgs.arguments = args.entryData.args;
	}
	catch(err)
	{
 ...
 * @private
 */
Tracer.prototype.pushFrame = function(stackFrame, args ) {
	// Need a unique id for a stack (in case of recursive calls the stackFrame would be the same).
	var stackId = idgen();
	var stackFrameObject = new Object();

	stackFrameObject.stackFrames = stackFrame;
	stackFrameObject.stackArgs = args;
	this.stack.stackMap[stackId] = stackFrameObject;
