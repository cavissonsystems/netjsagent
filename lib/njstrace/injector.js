...
var falafelPath,falafelVersion=0,options={};
	falafelPath= path.dirname(require.resolve('falafel'));
	try {
		falafelVersion = require(falafelPath + '/package.json').version;
	}catch(e){falafelVersion='2.1.0'}
	options = compareVersion(falafelVersion,'1.0.0') === -1? {range: true, loc: true} : {ranges: true, locations: true}
		...
		var output = falafel(code, options, function (node) {
			...
			if (name) {
				try {
					...
					var methodName = dirName + "." + baseName + '.' + name + ';'+(node.loc.start.line) ;
