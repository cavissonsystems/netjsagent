...
var TRACE_EXIT = '__njsTraceExit__({file: __njsEntryData__.file,name: __njsEntryData__.name,cavMthStartTime:__njsEntryData__.cavMthStartTime,flowPathObj:__njsEntryData__.flowPathObj,methObj:__njsEntryData__.methObj,methodId:__njsEntryData__.methodId,cavIncludeFp:__njsEntryData__.cavIncludeFp,exception: %s, line: %s, returnValue: %s, args: %s});';
		...
		var output = falafel(code, {range: true, loc: true}, function (node) {
			...
			if (name) {
				try {
					...
					var traceEntry = util.format(TRACE_ENTRY, JSON.stringify(relPath), JSON.stringify(name), node.loc.start.line, args);
