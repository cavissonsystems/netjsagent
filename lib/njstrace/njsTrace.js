 ...
 */
NJSTrace.prototype.addModuleInList = function(modulesList) {

    var modules = Object.keys(modulesList);
    /*
    ...
    if (modules.length > 0)
    {
        for(i in modules)
        {
            DEFAULT_CONFIG.files.push(modules[i]);
        }
    }
}

NJSTrace.prototype.inject = function(config,instrumentationFile) {
    ...
    try {
        ...
        if(instrumentationFile) {
            ...
            DEFAULT_CONFIG.files.push('!**/node_modules/netjsagent/**')
        }
        console.log(DEFAULT_CONFIG.files)
    }
    catch(err){console.log(err)}


    /*if (fs.existsSync(instrumentationFile)) {
        var data = fs.readFileSync(instrumentationFile).toString().split("\n");
...
NJSTrace.prototype.hijackCompile = function() {
	...
	Module.prototype._compile = function(content, filename) {
        try {
            ...
            //console.log(process.cwd(),"      ",path.resolve(path.join(__dirname,'/../../../../')))

            var instrument = fnmatch(relPath, self.config.files);

            if (instrument) {
               // cavUtils.logger.info('Instrumenting:', filename);
...
NJSTrace.prototype.setGlobalFunction = function() {
	...
	global.cavisson_isArgumentContainReq = function(args)
	{
		...
		{
			//console.log("Error in getting cavisson arguments : " + err)
		}
	}
};
