       ...
       //         util.logger.error(agentSetting.currentTestRun+" | Control connection, Received end event with retrying : "+retrying+", on socket-  " + err);
        ...
        client.on('close', function(err) {
            try {
                ...
                util.logger.error(agentSetting.currentTestRun+" | Control connection, Received socket close event from Host : ",agentSetting.getNDCHost()," ,Port : ",agentSetting.getPort(),' ,error : ', err);
                connectToServer();
            }
            catch(err){util.logger.warn(agentSetting.currentTestRun+" | Error in retrying for control connection with Host : ",agentSetting.getNDCHost()," ,Port : ",agentSetting.getPort(),' ,error : ', err);}
        });
