...
{
    if(client != null && client.writable )
        return;

    if(socket_timeout)
        return;

    util.logger.warn(agentSetting.currentTestRun + ' | @client.js->connectToServer() please Wait for '+waitTime/1000+' Seconds');
...
clientConn.closeConn = function(){
    try{
        ...
        util.logger.warn(agentSetting.currentTestRun+" | Error in client CloseConn()" + err);
    }
}
