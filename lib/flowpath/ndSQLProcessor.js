...
ndSQLProcessor.dumpNonPreparedSQLQueryEntry= function (command,flowpathId,queryId)
{
    ...
    util.logger.info("Dumping MetaRecord for Query : " + queryMetaData);
    try {
        if (AgentSetting.isToInstrument && AgentSetting.dataConnHandler) {
            AgentSetting.dataConnHandler.client.write(queryMetaData);
        }
    }
    catch (err) {
...
//moexports.ndSQLProcessor = ndSQLProcessor;