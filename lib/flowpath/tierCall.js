...
tierCall.prototype.generate_T= function (sb) {
    //T5:3:1:2:1:0_
    //if(this.seqId && this.methodId && this.executionTime &&this.backendType &&this.queryStartTimeMills  && this.statusCode) {
        if (this.status == 0) {
            sb.add('T')
        ...
        if (this.subType) {
            sb.add(this.subType)
            sb.add(':')
            sb.add(this.queryStartTimeMills)
        }
        else {
            sb.add('-')
            sb.add(':')
        ...
        if (this.statusCode > 0 && this.status == 1) {
            sb.add(':');
