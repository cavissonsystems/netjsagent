...
function checkAndCreateDir(logDir,os,pid,instanceName,logLevel){
    ...
    * */
    if(instanceName) {
        ...
        for( i in dir){
            if (dir[i].indexOf(pid) !== -1) {
                ...
                if(os == 'window'){
                    ...
                    fs.renameSync(curr,newFile)
                    return newFile;
                }
                else if(os == 'linux'){
                    curr = logDir+'/'+file;
                    newFile= logDir+'/nd_node_'+logLevel + '_'+instanceName+'.log'
                    fs.renameSync(curr,newFile)
                    return newFile;
                }
            }
        ...
        if(os == 'window')
            return logDir+'\\nd_node_'+logLevel+'_'+pid+'.log';
        else if(os == 'linux')
            return  logDir+'/nd_node_'+logLevel + '_'+pid+'.log'
    }
}

util.initializeLogger = function(logLevel,BCILoggingMode,instanceName)
{
    try {
      /*  if(BCILoggingMode)
            BCILoggingMode='FILE';*/
        //if(!instanceName)
        //    instanceName = process.pid
        var machine = os.type(),
            logDir,
            filename
            ...
            ({
                transports: [
                    new (winston.transports.File)({
                        ...
                        name: 'info-file',
                        filename: filename,      // file for both output
                        level: logLevel,
                    ...
                    new (winston.transports.File)({
                        ...
                        name: 'error-file',
                        filename: filename,            // file only in case of error
                        level: 'warn',
