...
flowpathhandler.clearCounter =function(){old=0;};
...
flowpathhandler.ceateFpId = function()
{
    try {
        ...
        var mask_counter = (big_integer(big_integer(big_integer(currTimeStamp).and(agentSetting.timeStampMask)).shiftLeft(agentSetting.seqNoDigits)).add(big_integer(old).and(agentSetting.seqNumMask.toString()))).toString()
