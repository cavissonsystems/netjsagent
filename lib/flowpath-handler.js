    ...
    if (req.hasOwnProperty('url') /*&& (header.indexOf('text/html') > -1 || header.indexOf("*!/!*")  > -1)*/ ) {
        ...
        if (requestMap[url] == undefined)
        {
            try {
                ...
                util.logger.warn(agentSetting.currentTestRun+" | Error in flowpath-handler.js is :"+err);
            }
        }
        else {
            id = requestMap[url];
        ...
        try
        {
            ...
            util.logger.warn(agentSetting.currentTestRun+" | Error in creating flowpath : "+err);}
    }
}
