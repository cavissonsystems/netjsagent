...
function handleIncludedAndExcludedCase (context){
    ...
    if (curRandomNumber <= agentSetting.bciInstrSessionPct) {
        if(agentSetting.enableBciDebug>2) {
            util.logger.info('Enable flow path dump because random number generated is less than Instr percentage ', agentSetting.bciInstrSessionPct)
            global.totDumped++;
        }
...
function setCookieInResp(cookie,req,res,cavNVCookie,isInstrumentedSession){
    var cookieValueObj = {};  //passing object as third arguments for set.
    cookieValueObj["path"]="/";
    if(agentSetting.enableNDSession.cookieDomainName){
        cookieValueObj["domain"]=agentSetting.enableNDSession.cookieDomainName;
    }
    cookie
        .set(agentSetting.enableNDSession.fPIHeaderName , cavNVCookie, cookieValueObj);

    if(agentSetting.enableNDSession.setHeaderInResponse){
        var xHeaderValue = fpid +'_'+ agentSetting.currentTestRun +'_'+ '9' +'_'+ (isInstrumentedSession?1:0) ;
...
function checkFlowpathToIncludeOrExclude(cavNVCookie,context,isInstrumentedSession,isSessionExpired){
    if ('100' == agentSetting.bciInstrSessionPct) {
        context.cavIncludeFp = true;
        if(agentSetting.enableBciDebug>2)
            util.logger.info('Enable flow path dump because bciInstrumentationPct 100 ');
        ...
        if(isInstrumentedSession){
            if(!isSessionExpired){
                ...
                if(agentSetting.enableBciDebug>2) {
                    global.totDumped++;
                    util.logger.info('Enable flow path dump because NV Session expired for instrumented flow path');
                }
            }
            else{
                if(agentSetting.enableBciDebug>2)
                    util.logger.info('Session Instrumented But Expired')
            ...
            if(!cavNVCookie){
                if(agentSetting.enableBciDebug>2)
                    util.logger.info('Not Instrumented and !cavNVCookie')
...
flowpathhandler.handleFlowPath = function(req,res,context) {
    ...
    if (req.hasOwnProperty('url') /*&& (header.indexOf('text/html') > -1 || header.indexOf("*!/!*")  > -1)*/ ) {
        ...
        //for counting all the transaction
        if(agentSetting.enableBciDebug > 2){
            global.totTran++;
        }
        ...
        if(!fpid){
            util.logger.error(agentSetting.currentTestRun, "| Unable to generate fp id.")
            return;
        }
        ...
        agentSetting.checkforCorrelationIdInReqRes(req, Flowpath);
        var NVSid = undefined,cookies = null,NVPid = undefined,cavNVCookie = null,nvCokkie= null,ndSessionId =0,
        ...
        if(agentSetting.enableNDSession.enableFPHdrInRep) {
            ...
            if(nvCokkie){
                var temp = nvCokkie.toString().split('-')
                if(temp.length >= 2) {
                    NVSid = temp[0];
                    NVPid = temp[1];
                }
        ...
        if(!cavNVCookie) {
            ...
            if(agentSetting.enableBciDebug > 3)
                util.logger.info(agentSetting.currentTestRun + ' | Fetching cookie from the Request Header for Url : ',url,' ,FPID : ',fpid ,' With Value : ',cavNVCookie)
            if(cavNVCookie.indexOf(",") !== -1) {
                ...
                ndSessionId = (tmpStr[NDNV_SESSION_ID_INDEX]?tmpStr[NDNV_SESSION_ID_INDEX].trim():fpid);
                ...
                prevFPStartTime = parseInt(tmpStr[NDNV_PREV_START_TIME_INDEX]?tmpStr[NDNV_PREV_START_TIME_INDEX].trim():0);
                isInstrumentedSession = tmpStr[NDNV_IS_INSTRUMENTED_SESSION_INDEX] ? (tmpStr[NDNV_IS_INSTRUMENTED_SESSION_INDEX].trim() === "1" ? true: false) :false

                idleTime = (new Date().getTime() - agentSetting.cavEpochDiffInMills) - prevFPStartTime;
                totalFPCountInSession = parseInt(tmpStr[NDNV_FP_COUNT_IN_SESSION_INDEX]?tmpStr[NDNV_FP_COUNT_IN_SESSION_INDEX].trim():0);
        ...
        flowpathhandler.checkIsNSGeneratedFP(nsFlowpathInstanceID,Flowpath, context)
        flowpathhandler.checkForForcedFpChain(context,Flowpath)
        isInstrumentedSession = context.cavIncludeFp
        Flowpath.ndSessionId = context.ndSessionId = ndSessionId;
        Flowpath.NVSid = context.NVSid = NVSid; //set these values in flowpath Object.
        Flowpath.NVPid = context.NVPid = NVPid;
        ...
        if(agentSetting.enableBciDebug > 3)
            util.logger.info(agentSetting.currentTestRun + ' | Setting Cookie in Respose Header For Url : ', url , '& FPID : ',fpid ,' with Value : ',cavNVCookie)
        ...
        if(!cavNVCookie) {
            if (!bt_Data) {
                ...
                if (bt_Data.isDumpPctLessThanBCIPct(agentSetting.bciInstrSessionPct)) {
                    ...
                    if(agentSetting.enableBciDebug>2) {
                        global.totDumped++;
                        util.logger.info('Enable flow path dump because percentage of flow path dumped for BT is less than Instr percentage ', btObj.btName)
                    }
                    context.cavIncludeFp = true;
                ...
                if (bt_Data.isDumpPctLessThanBCIPct(agentSetting.bciInstrSessionPct)) {
                    ...
                    if(agentSetting.enableBciDebug>2) {
                        util.logger.info('Enable flow path dump because percentage of flow path dumped for BT is less than Instr percentage ', bt_Data.BTName)
                        global.totDumped++;
                    }
                }
                else{
        ...
        * */
        ...
        Flowpath.cavCurrReqFPID = context.cavCurrReqFPID = fpid;
        context.cavHsFlowPathId = fpid;
        context.seqId = 0;              //Initializing Sequence id
...
flowpathhandler.checkIsNSGeneratedFP = function(nsFlowpathInstanceID,Flowpath,context){
    /*
       * Checking for NS genertated FPID
       * */
    if(nsFlowpathInstanceID)
    {
        //In case of enbaleCaptureNetworkDelay feature true, removing the time stamp
        if(nsFlowpathInstanceID.indexOf("#") > -1){
            nsFlowpathInstanceID = nsFlowpathInstanceID.split('#')[0]
        }

        if(nsFlowpathInstanceID){
            if(nsFlowpathInstanceID.indexOf("_") == -1) {
                if(nsFlowpathInstanceID == "F" || nsFlowpathInstanceID == "f"){
                    Flowpath.tlFirstTierFPID = fpid+nsFlowpathInstanceID;
                    flowPathId = fpid;
                    context.cavIncludeFp = true;

                    if(agentSetting.enableBciDebug>2) {
                        util.logger.info('Enable flow path dump because nsFlowpathInstanceID value is F or f ', nsFlowpathInstanceID)
                        global.totDumped++;
                    }
                }else{
                ...
                if(tlFirstTierFPID)
                    ...
                    Flowpath.tlFirstTierFPID = fpid;
                // }
            }
        }
        else{
            flowpathhandler.noChainedFpFound(Flowpath);
            util.logger.error(agentSetting.currentTestRun, "| No value is received from cavndfpinstance header.")
        }
    }
    else{
        flowpathhandler.noChainedFpFound(Flowpath);
...
flowpathhandler.noChainedFpFound = function(Flowpath){
...
flowpathhandler.checkForForcedFpChain = function(context,Flowpath){
    if(Flowpath.tlFirstTierFPID.indexOf('F') !== -1){
        context.cavIncludeFp = true;
        if(agentSetting.enableBciDebug>2) {
            util.logger.info('Enable flow path dump because Flowpath.tlFirstTierFPID contains F ', Flowpath.tlFirstTierFPID)
            global.totDumped++;
        }
    }
    else if(Flowpath.tlFirstTierFPID.indexOf('f') !== -1 ){
        if(!context.cavIncludeFp)Flowpath.dumpForcefullL1FP = true;
    }
    else{
        if(context.cavIncludeFp == true)
            agentSetting.enableForcedFPChain > 1 ? Flowpath.tlFirstTierFPID += 'F' : (agentSetting.enableForcedFPChain == 1 ? Flowpath.tlFirstTierFPID += 'f' : Flowpath.tlFirstTierFPID += '');
    }
}
