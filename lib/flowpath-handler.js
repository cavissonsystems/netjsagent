...
flowpathhandler.ceateFpId = function()
{
    try {
        ...
        util.logger.info("New fpid has been created :" +fpid);

    }catch(err){util.logger.warn(err)}
}

flowpathhandler.clearURLMap = function()
{
    ...
    if (req.hasOwnProperty('url') /*&& (header.indexOf('text/html') > -1 || header.indexOf("*!/!*")  > -1)*/ ) {
        ...
        if (requestMap[url] == undefined)
        {
            try {
                ...
                util.logger.warn("Error in flowpath-handler.js is :"+err);
            }
        }
        else {
            id = requestMap[url];
        ...
        try
        {
            if (agentSetting.isToInstrument && agentSetting.dataConnHandler)
            {
               threadID =  Math.floor(Math.random() * (999 - 1 + 1) + 1);
                ...
                //agentSetting.dataConnHandler.client.write(encoded2_record);
            }
