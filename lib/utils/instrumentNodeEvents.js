...
function makeWrappedCallback(callback, sourcePosition, frameLocation, eventName,parentId) {
    ...
    return function () {
        ...
        if (args.length == 0) {
            ...
            args[args.length] = ['Source - '+sourcePosition + '; Event - ' + eventName + (parentId ? '; parentId - '+parentId :'')+ '; Delay:' + time]
        else
            args[args.length] = ['Source - '+sourcePosition + (parentId ? '; parentId - '+parentId :'')+ '; Delay:' + time]
