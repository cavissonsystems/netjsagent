...
var global=0,instrumentAsyncFunc=false;
        ...
        if(typeof args[len-1] != 'function') {
            if (len == 0) {
                if (/on|addListener|once/g.test(frameLocation))
                    args[1] = ['Source - ' + sourcePosition + '; Event - ' + eventName + (parentId ? '; parentId - ' + parentId : '') + '; Delay:' + time]
                else
                    args[1] = ['Source - ' + sourcePosition + (parentId ? '; parentId - ' + parentId : '') + '; Delay:' + time]            // first argument will always be error, so dding value at last index
            } else if (/on|addListener|once/g.test(frameLocation))
                args[len] = ['Source - ' + sourcePosition + '; Event - ' + eventName + (parentId ? '; parentId - ' + parentId : '') + '; Delay:' + time]
            else
                args[len] = ['Source - ' + sourcePosition + (parentId ? '; parentId - ' + parentId : '') + '; Delay:' + time]
        }
        try {
    ...
    if (instrumentAtStart || global > 0) {
        object[property] = function () {
            ...
            if (typeof orig === 'function' ) {
                if(!/request|close|error/g.test(arguments[0]))
...
module.exports = function(isEnable,instrAsyncFunc,default_instr) {
    global=0,instrumentAsyncFunc=false
