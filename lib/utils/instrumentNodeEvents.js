        ...
        object[property] = function () {
            ...
            if(isInstrumentTx) {
                ...
                if (instrumentAsyncFunc) {
                    var cav_args = {eventName : undefined, methodName:undefined, parentId : undefined ,args : undefined};
                    ...
                    try{
                        var context = agent.getContextObj()
                        if(context){
                            cav_args.parentId = ++context.seqId;
                            if(!!agent.enableDumpAsyncId)
                                global.cavisson_correlation_asyncId=cav_args.parentId;
                        }
                        mm.onEntry(cav_args)            // call the original registration function with the modified arguments
                        global.cavisson_correlation_asyncId = undefined
                    }catch (e){
                        util.logger.error(agent.currentTestRun,' | Error in wrapRegistrationFunction',e)
                        global.cavisson_correlation_asyncId = undefined
                    }
...
module.exports = function(isEnable,instrAsyncFunc) {
    ...
    if(isEnable == 0 || isEnable == undefined || isEnable == null)return
