...
const VERBOSE_ENCODING = 1;
const SMALL_ENCODING = 2;
...
function TDigest(delta, K, CX) {
    ...
    // 0 to use exact quantiles for each new point.sandeep
    //
...
TDigest.prototype.byteSize = function(){
    return 4 + 8 + 4 + this.centroids.size * 12;
}

TDigest.prototype.smallByteSize = function(){
    var bound = this.byteSize();
    var buf = new Buffer.allocUnsafe(bound);
    var buff = this.asSmallBytes(buf,0);
    return buff;
}
...
TDigest.prototype.asBytes = function(buf,offset) {

    try{
        var a1 = buf.writeInt32LE(VERBOSE_ENCODING,offset);
        a1 = buf.writeDoubleLE(100.0,a1);
        a1 = buf.writeInt32LE(this.centroids.size,a1);

        this.centroids.each(function(c) {
            a1 = buf.writeDoubleLE(c.mean,a1)
        });

        this.centroids.each(function(c) {
            a1 = buf.writeInt32LE(c.n,a1)
...
TDigest.prototype.asSmallBytes = function(buf,offset){

    try{
        var a1 = buf.writeInt32LE(SMALL_ENCODING,offset);//4 bytes
        a1 = buf.writeDoubleLE(100.0,a1); //8 bytes
        a1 = buf.writeInt32LE(this.centroids.size,a1); // 4 bytes

        var x = 0;
        this.centroids.each(function(c){
            var delta = c.mean - x;
            delta.toFixed(5);//testing with upto 5 decimal points
            x = c.mean;
            a1 = buf.writeFloatLE(parseFloat(delta),a1);//4 bytes
