  ...
  return function () {
	...
	if(context['startTime']){
                var duration=(new Date().getTime()-context['startTime']);
                 if(duration > asSettingObj.asSampleInterval*asSettingObj.asThresholdMatchCount){ //comparing duration of event with asThreshold
                    ...
                    //if(storage['httpReq'] && storage['httpReq'].flowPathId){  //If flowpathid and httpRequest object is not noll than get the fpid.
                    //flowPathId=storage['httpReq'].flowPathId;
                    //storage['httpReq'].flowPathId=null;  //After getting FPId , assign it to null other wise it overridden for other events and methods.
                    //}
                 if(flowPathId === undefined)
                   flowPathId = '0';
		         //calling hotspot method to create 52 and 53 recods.
                 asManagerFile.handledHotspotData(stack, duration, (context.startTime - agentSetting.cavEpochDiffInMills),flowPathId,"", "200", (new Date().getTime() - agentSetting.cavEpochDiffInMills));
               }
    }
    try {
  ...
  namespace.id = process.addAsyncListener({
    create : function () {
		          if(namespace.active) {
					...
					namespace.active['stack']= new Error().stack;
				} 
      ...
      if(storage['startTime']){ //This code used to detect HotSpot events asynchronously, startTime is setting in bind function when call invokes.
        ...
        if(duration > asSettingObj.asSampleInterval*asSettingObj.asThresholdMatchCount){ //comparing duration of event with asThreshold
          ...
          if(flowPathId === undefined)
            flowPathId = '0';
		  //calling hotspot method to create 52 and 53 recods.
          asManagerFile.handledHotspotData(stack, duration, (storage.startTime - agentSetting.cavEpochDiffInMills),flowPathId,"", "200", (new Date().getTime() - agentSetting.cavEpochDiffInMills));
        }
      }
